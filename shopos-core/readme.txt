...
Unified WooCommerce toolkit for the ShopOS Theme. Nine modules in a single plugin with a single admin surface.

== Description ==

ShopOS Core replaces six separate WooCommerce plugins with a single modular plugin. Each feature is independently togglable, so you only run what you use. The plugin owns its own data — deactivating the ShopOS Theme never orphans a subscriber list or settings.
...
= 1.0.3 =
Swatches: simple products now use the ShopOS buy box on both the PDP (Add to Cart + Buy Now + quantity + sticky mobile bar) and the shop grid (compact AJAX add-to-cart with quantity stepper). Out-of-stock simple products show a disabled ShopOS button.

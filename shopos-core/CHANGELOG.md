...
- **B-5 dead-code sweep — dead localize payloads (audit-D §2).** Removed four `wp_localize_script` payload keys that no JS ever read (grep-verified, no reader in each module's JS): **ShopFilters** `Shortcode::enqueue_assets()` dropped the entire `ShopOSShopFilters` payload (`ajaxUrl`/`action`/`nonce`/`contextId`) — the panel uses the reload transport (full-page nav), not admin-AJAX, so this also removes a wasted per-render `wp_create_nonce`; the now-unused `$context_id` param + arg were removed too. **BundleDeals** `Frontend::localized_payload()` dropped its `labels` sub-array (`remaining`/`unlocked`) — the mix-&-match wording is rendered server-side, so the JS never read it. **Search** `Frontend::localized_payload()` dropped the `searching` label. **RestockNotify** `Frontend` dropped the `scriptMissing` i18n key (frontend.js uses a hardcoded fallback for that guard). No behaviour change — every removed key was inert. Label default maps are untouched (the strings stay available). Also removed two dead-code artifacts (owner-approved 2026-07-22): the **`shopos_restock_notification_log` accumulator writer** in the modern `Stock_Monitor::notify()` — the option was written (capped 100) but nothing ever read it, and its docblocks claimed a "recent-notifications widget" that doesn't exist (the legacy `Stock_Monitor` twin is aliased-over and never loaded, so left untouched; the option is already dropped on uninstall in 1.56.0) — and the orphan **`.shopos-toast` CSS block** (~90 lines) in `shopos-swatches.css`, which had no emitter (the shipped toast is `.shopos-shop-toast`). Dropped the now-obsolete `RestockNotifyStockMonitorTest::test_notification_log_appends_and_caps_at_100`. `baseline-hooks.txt` position-only shift; baselines otherwise unchanged (the `notification_log` literal is still declared by the 1.56.0 uninstall delete); `.pot` header-only. **1099 tests / 3448 assertions green (php@8.3).** Version bumps manual; readme.txt Stable tag left at 1.52.0. Rollback: revert the commit.

## [1.56.0] — 2026-07-22

- **B-5 uninstall completeness (audit-D).** Closes the uninstall leaks that fall outside the per-module `shopos_core_<id>_%` option sweep. **RestockNotify** `on_uninstall()` now **DROPs the `{prefix}shopos_restock_subscribers` PII table** (customer emails) and deletes the off-prefix `shopos_restock_notification_log` accumulator — WordPress only runs uninstall on an explicit "Delete", a deliberate data-destroying action (owner-approved 2026-07-22; Hard Rule #3 legacy-surface edit — the table name is built from the trusted `$wpdb->prefix`, no legacy class loaded). **VariationSwatches** gains an `on_uninstall()` that clears the `_shopos_core_vs_sampled_color` post-meta the color sampler writes on every variation (thousands of rows the option-only sweep missed). **`uninstall.php`** explicit-delete list extended with the **Core-owned** options no id-prefix sweep reaches: `shopos_core_log`, `shopos_core_settings_backups`, the whole `shopos_core_design_*` family (one `LIKE` pass), and `shopos_core_productfeed_last_generated` (whose `productfeed` key never matched the module's `product_feed_%` sweep). The audit's ProductFeed/VariableStockFix "orphan cron" finding was **verified already covered** — both modules clear their crons in `on_deactivate()`, which always runs before the Delete→uninstall path — so no redundant code was added. New `UninstallCleanupTest` (RestockNotify DROP + notification-log delete via a recording `$wpdb`; VariationSwatches meta-key sweep) + a `delete_post_meta_by_key` bootstrap stub; the procedural `uninstall.php` option deletes are live-QA. Baselines unchanged (no captured-hook shift); `.pot` header-only. Version bumps manual; readme.txt Stable tag left at 1.52.0. **1100 tests / 3450 assertions green (php@8.3).** Rollback: revert the commit — uninstall behaviour only runs on plugin delete.

## [1.55.0] — 2026-07-22

- **New module: Side Cart** (`side_cart`, default OFF — absent from `shopos_core_modules`, the PageTransitions/Bundle Deals pattern; the module-enable toggle is the single kill-switch). A slide-out cart drawer that opens on add-to-cart (WooCommerce's jQuery `added_to_cart` event) and when a cart link is clicked, showing line items with a live subtotal, a free-shipping progress meter, a coupon field, remove-with-undo, cross-sell recommendations and a quick checkout button. **Thin over WooCommerce:** the drawer body renders from `WC()->cart`; recommendations are the cart's own `get_cross_sells()`; the body is registered as a WC cart fragment (`woocommerce_add_to_cart_fragments`) so fragment-refreshing stores keep it live; checkout/view-cart use `wc_get_checkout_url()`/`wc_get_cart_url()`. One public AJAX endpoint `shopos_core_side_cart` (nonce + per-IP rate limit) with an `op` selector — `refresh | apply_coupon | remove_coupon | remove_item | restore_item` — each delegating to the WC cart API and returning the freshly rendered body (remove/undo rides WC's own `remove_cart_item`/`restore_cart_item`). Skin-light `side-cart.css` (all `--shopos-ui-*` tokens with on-value fallbacks, RTL-correct logical properties, 640/1024 canon, reduced-motion) head-enqueued FOUC-free; progressive-enhancement `side-cart.js` (no jQuery required for the drawer; binds `added_to_cart` only when jQuery is present). Every storefront string overridable via `Labels` (`shopos_core_side_cart_label_*`). Two behaviour toggles (`free_shipping_meter`, `show_recommendations`). Additive filters `shopos_core/side_cart/free_ship_min`, `/recommendations`, `/fragments` (no feature flag — Hard Rule #1 additive exception; module toggle is the kill-switch). New `Meter` (pure free-shipping math), `SideCartMeterTest` / `SideCartModuleTest` / `SideCartAjaxTest` (the cart-driven body render + coupon/remove ops are integration — live-QA). `baseline-hooks.txt` +3 additive; `baseline-options-declared.txt`/`-legacy.txt` +side-cart literals; `.pot` regenerated. **1098 tests / 3446 assertions green (php@8.3).** Rollback: `wp option patch update shopos_core_modules side_cart 0` (or the module toggle). Version bumps manual (release.sh zip build at ship time); readme.txt Stable tag left at 1.52.0 (publish is a separate owner decision).

## [1.54.0] — 2026-07-21

- **ShopOS v2 design migration ("The Quiet Boutique") — core-side.** The design-system upgrade to DESIGN.md v2, executed as the §20 migration checklist. The token/primitive layer lives in `shopos-theme`; these are the `shopos-core` changes that ride it. All backward-compatible (module CSS consumes tokens with on-value fallbacks; no flags flipped; no hook/option/REST/CLI surface changes). **1069 tests / 3363 assertions green (php@8.3).**
  - **Design panel:** the accent-preset **forest** repaint aligned to `#3f6b4f` (was `#3f7a4b`) to match the new `--shopos-ui-palette-forest` token; `Design::KIT_SLOT_DEFAULTS` **display slot `sk_type_2` → `sk_type_12`** (v2 collapses to one type family, so display maps to the body slot). `DesignPanelTest` updated.
  - **Feature flag copy:** the `theme/fonts_selfhost` description reworded to one variable family (Heebo, weights 300–700); `.pot` regenerated.
  - **Module CSS/JS swept to the v2 doctrine** (all output-compatible): 768px breakpoints reconciled to the 640/1024 canon across VariationSwatches / ShopFilters / ProductPage / Search; `radius-sm` consumers migrated to the shape ladder (controls/thumbnails → `radius-xs`, notices → `radius-md`) in InfiniteScroll / MyAccount / RestockNotify / ProductPage / BundleDeals; per-component focus box-shadows replaced by the single `:focus-visible` ink outline (RestockNotify + ProductPage); compare-at `<del>` prices moved to **ink-muted, not red** (Semantic-Only corollary) in ProductPage + BundleDeals; status `-soft` fills now use the matching `-text` token (ProductPage stock-urgency, BundleDeals, RestockNotify error); one-family font fallback stacks in ProductPage. RestockNotify inline-critical-CSS goldens regenerated.
  - **iconbtn adoption:** QuickView card trigger + drawer close adopt the new `.shopos-ui-iconbtn` primitive (transparent, no shadow, tonal hover — verified in wp-env); Search trigger recipe-aligned in place (its Elementor-header `!important` armor precludes the shared class). `QuickViewFrontendTest` trigger-class assertion updated.
  - **tokens:check drift guard:** new `tools/tokens-check.php` (diffs the DESIGN.md frontmatter against `shopos-tokens.css`, 71 tokens) wired into the test gate via new `TokensParityTest`.
  - Icon-trigger adoption for the slider `.cs-arrow` (operator CSS-var controls), card-slider/gallery arrows, ShopFilters close/chip, and the ShopOS Line PLP/PDP structural relayout are documented follow-ons gated on the owner's live store (DESIGN.md §20).

## [1.53.0] — 2026-07-20

- New feature flag **`theme`/`style_emails`** (`shopos_core_theme_style_emails_enabled`, default OFF, permanent kill-switch — decisions §11 Ruling 4) in `Feature_Flags::registry()`. Gates the ShopOS Line's **sixth and final §11-B deferred surface**: the transactional-email skin — **this closes §11-B**. Unlike every sibling surface it is **Core-side, not theme-side**: WooCommerce emails send from cron / webhook / REST contexts where the active theme may not be ShopOS Line, so a theme-level email override would vanish (decisions §11 line 304). New `ShopOS\Core\Core\Email_Skin` is booted from `Plugin::boot()` at method scope (so it registers in cron/REST send contexts, not only admin) **only when the flag is on**. Skin-only like the checkout surface — it hooks `woocommerce_email_styles` and **appends** email-safe CSS (literal hex/px values; no CSS custom properties, `@media`, or logical properties, which email clients drop) that WooCommerce inlines onto the email markup via Emogrifier; it forks **no** email templates, so there is no WooCommerce email `@version` to chase and WooCommerce keeps ownership of every email template/header/footer (Core only appends brand styles, never replacing WooCommerce's). Off = the WooCommerce default email styling with nothing appended, byte-identical (Ruling 6). Core-only: does NOT need the ShopOS theme active. New `EmailSkinTest` (`@group theme`, 5 cases: frozen flag name, registry entry, `boot()` hooks `woocommerce_email_styles`, appended-not-replacing, and the email-safety contract). Blueprint / admin / CLI coverage auto-derives from the registry. Rollback: `wp option update shopos_core_theme_style_emails_enabled 0`.

## [1.52.0] — 2026-07-20

- New feature flag **`theme`/`style_checkout`** (`shopos_core_theme_style_checkout_enabled`, default OFF, permanent kill-switch — decisions §11 Ruling 4) in `Feature_Flags::registry()`. Gates the ShopOS Line's fifth §11-B deferred surface: the theme checkout skin. **This resolves the Ruling 9 checkout technology pin as MOOT** (owner, 2026-07-20): unlike the cart/account surfaces the checkout surface forks **no** templates — the theme enqueues its own `shopos-checkout.css` on the checkout page and only restyles WooCommerce's markup by its stable classes, so WooCommerce keeps ownership of every checkout field, nonce, and payment gateway (the My Account CSS-skin doctrine applied to the whole page). Because it is skin-only it works on **both** the shortcode (`[woocommerce_checkout]`) and the block checkout, so `cart_checkout_blocks` stays declared `true` and **no per-store content-migration is ever required**. Off = the current checkout render with no extra assets, byte-identical (Ruling 6). Blueprint / admin / CLI coverage auto-derives from the registry. Pairs with shopos-theme 1.20.0.

## [1.51.0] — 2026-07-20

- New feature flag **`theme`/`template_search`** (`shopos_core_theme_template_search_enabled`, default OFF, permanent kill-switch — decisions §11 Ruling 4) in `Feature_Flags::registry()`. Gates the ShopOS Line's fourth §11-B deferred surface: the theme-owned product search-results page. The theme renders its own classic `templates/woo/search-results.php` via the shared `template_include` loader — a new claim arm alongside the PLP arm, resolved only when the request is a product-archive main query carrying a search term (the mirror-positive of the PLP loader's §11 Ruling 2 search refusal; a generic post/page search is never claimed, matching the Search module's `Results_Query` gate). Never by file presence (§11.3). The always-on `Results_Query` already supplies the engine-ranked results, so the template only reskins the surrounding archive. Off = the current search render, byte-identical (Ruling 6). Blueprint / admin / CLI coverage auto-derives from the registry. Pairs with shopos-theme 1.19.0.
...
- New feature flag **`theme`/`template_chrome`** (`shopos_core_theme_template_chrome_enabled`, default OFF, permanent kill-switch — decisions §11 Ruling 4, exempt from graduation sweeps) in `Feature_Flags::registry()`. Gates the ShopOS Line's first §11-B deferred surface: the theme-owned header/footer chrome (the theme renders its own classic header + footer instead of the Elementor Theme Builder / Hello Elementor chrome). Off = the theme passes through to the parent chrome, byte-identical. Blueprint / admin / CLI coverage auto-derives from the registry (zero test edits beyond the registry-iterating counts). Pairs with shopos-theme 1.16.0.

...
final class Module extends Module_Base {
	...
	public function label() {
		return __( 'Cheapest Default Variation', 'shopos-core' );
	}
	...
	public function description() {
		return __( 'Auto-selects the cheapest in-stock variation as the default so customers can add to cart without picking options.', 'shopos-core' );
	}
	...
	public function settings_schema() {
		return array(
			'respect_manual_defaults' => array(
				'label'          => __( 'Respect manual defaults', 'shopos-core' ),
				'type'           => 'checkbox',
				'checkbox_label' => __( 'Leave defaults set in the product editor alone', 'shopos-core' ),
				'description'    => __( 'When on, manually chosen defaults take precedence over this automatic selection.', 'shopos-core' ),
				'default'        => 1,
			),
			'pdp_only'                => array(
				'label'          => __( 'Apply on product pages only', 'shopos-core' ),
				'type'           => 'checkbox',
				'checkbox_label' => __( 'Skip this auto-selection on shop / archive / loop contexts', 'shopos-core' ),
				'description'    => __( 'When on, archive and shop-loop swatches/pickers render with no pre-selected variation — the customer has to actively pick one. The single-product page (PDP) still auto-selects the cheapest.', 'shopos-core' ),
				'default'        => 1,
			),
			'strategy'                => array(
				'label'       => __( 'Default selection strategy', 'shopos-core' ),
				'type'        => 'select',
				'choices'     => array(
					'cheapest'       => __( 'Cheapest in-stock variation', 'shopos-core' ),
					'first_in_stock' => __( 'First in-stock variation (in WooCommerce order)', 'shopos-core' ),
				),
				'default'     => 'cheapest',
				'description' => __( 'Which variation gets pre-selected. "Cheapest" scans display_price; "First in-stock" returns the first variation WooCommerce returns that is in stock and purchasable.', 'shopos-core' ),
			),
		);
	}
	...
	public function default_cheapest_variation( $default_attributes, $product ) {
		 ...
		 */
		$should_apply = apply_filters( 'shopos_core/cheapest_variation/should_apply', true, $product, $default_attributes );
		if ( ! $should_apply ) {
			return $default_attributes;
		}
		 ...
		 */
		$cheapest = apply_filters( 'shopos_core/cheapest_variation/chosen', $cheapest, $product, $variations );
	...
	private function dispatch_strategy( $product ) {
		...
		$meta = get_post_meta( $product->get_id(), '_shopos_cheapest_variation_strategy', true );
		 ...
		 */
		$strategy = (string) apply_filters( 'shopos_core/cheapest_variation/strategy', $strategy, $product );

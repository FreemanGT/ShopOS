/**
 * ShopOS Infinite Scroll — product-grid auto-loader.
 *
 * Ported from shopos-infinite-scroll v1.0.5 with localization and
 * pluggable option values via window.ShopOSInfiniteScroll.
 *
 * Debug with ?shopos_is_debug=1 (legacy ?shopos_debug=1 also accepted).
 */
(function () {
    'use strict';

    var CFG = window.ShopOSInfiniteScroll || {};
    var DEBUG = /[?&](shopos_is_debug|shopos_debug)=1/.test(location.search);
    ...
    // setting + shopos_core/infinite_scroll/selector filter). When
     ...
     * --------------------------------------------------------------- */
    var RESTORE_PREFIX = 'shoposISRestore:';
    ...
    function log() {
        if (!DEBUG) return;
        console.log.apply(console, ['[ShopOS IS]'].concat([].slice.call(arguments)));
    }
    ...
    function buildFetchUrl(rawHref) {
        try {
            ...
            curU.searchParams.forEach(function (value, key) {
                if (key === 'shopos_is_debug' || key === 'shopos_debug') return;
    ...
    function insertSentinelAfter(container) {
        ...
        state.sentinel.className = 'shopos-infinite-sentinel';
        ...
        state.mainObserver = new MutationObserver(function (mutations) {
            ...
            for (var i = 0; i < mutations.length; i++) {
                ...
                for (var j = 0; j < m.removedNodes.length; j++) {
                    ...
                    if (n.classList && (n.classList.contains('shopos-skeleton') || n.classList.contains('shopos-end-message') || n.classList.contains('shopos-error-message'))) continue;
        ...
        fetch(urlBeingFetched, {
            ...
            .then(function (html) {
                ...
                for (var k = 0; k < unique.length; k++) {
                    unique[k].classList.add('shopos-new-product');
            ...
            .catch(function (err) {
                ...
                console.error('[ShopOS IS] fetch error:', err);
                removeSkeletons();
                showErrorMessage();
            })
    ...
    function applyHistoryMode(url) {
        ...
        try {
            ...
            if (mode === 'replaceState') {
                window.history.replaceState({ shoposPage: u.pathname }, '', pathSearch);
            } else {
                window.history.pushState({ shoposPage: u.pathname }, '', pathSearch);
            }
        } catch (e) { /* noop */ }
    }
    ...
    function measureCard() {
        ...
        for (var i = items.length - 1; i >= 0; i--) {
            var card = items[i];
            if (card.classList && card.classList.contains('shopos-skeleton')) continue;
    ...
    function makeSkeletonCard(measure) {
        ...
        skel.className = 'product shopos-skeleton wc-block-grid__product';
        skel.setAttribute('aria-hidden', 'true');
        skel.innerHTML = '<div class="shopos-skel-image"></div><div class="shopos-skel-line short"></div><div class="shopos-skel-line"></div><div class="shopos-skel-line price"></div>';
        if (measure) {
    ...
    function removeSkeletons() {
        if (!state.container) return;
        state.container.querySelectorAll('.shopos-skeleton').forEach(function (s) { s.remove(); });
    }

    function removeAuxMessages() {
        document.querySelectorAll('.shopos-end-message, .shopos-error-message').forEach(function (el) { el.remove(); });
    }
    ...
    function showEndMessage() {
        ...
        if (prev && prev.classList && prev.classList.contains('shopos-end-message')) return;
        var msg = document.createElement('div');
        msg.className = 'shopos-end-message';
    ...
    function showErrorMessage() {
        ...
        wrap.className = 'shopos-error-message';

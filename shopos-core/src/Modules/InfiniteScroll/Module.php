 ...
 *
 * Ported from shopos-infinite-scroll v1.0.5.
 *
 * @package ShopOSCore
 */
...
final class Module extends Module_Base {
	...
	public function label() {
		return __( 'Infinite Scroll', 'shopos-core' );
	}
	...
	public function description() {
		return __( 'Infinite scroll for WooCommerce product grids (shop, Elementor widgets, block grids) with skeleton placeholders and preserved /page/N/ URLs.', 'shopos-core' );
	}
	...
	public function settings_schema() {
		return array(
			'skeleton_count' => array(
				'label'       => __( 'Skeleton cards', 'shopos-core' ),
				'type'        => 'number',
				'description' => __( 'How many placeholder cards to show while loading.', 'shopos-core' ),
				'default'     => 6,
			),
			'max_pages'      => array(
				'label'       => __( 'Max pages', 'shopos-core' ),
				'type'        => 'number',
				'description' => __( 'Absolute safety limit — no more than this many pages will ever auto-load.', 'shopos-core' ),
				'default'     => 50,
			),
			'end_message'    => array(
				'label'       => __( 'End-of-list message', 'shopos-core' ),
				'type'        => 'text',
				'description' => __( 'Shown once there are no more products.', 'shopos-core' ),
				'default'     => __( 'You have reached the end.', 'shopos-core' ),
			),
			'trigger_mode'      => array(
			...
			'history_mode'      => array(
				...
				'type'        => 'select',
				'choices'     => array(
					'pushState'    => __( 'pushState — update URL and create back-button entries', 'shopos-core' ),
					'replaceState' => __( 'replaceState — update URL without back-button entries', 'shopos-core' ),
					'disabled'     => __( 'Disabled — leave URL unchanged on page advance (default)', 'shopos-core' ),
				),
				'default'     => 'disabled',
				'description' => __( 'How the browser URL is updated when an additional page loads. Disabled (default) keeps the URL on the archive, so Back from a product and shared links return to the full grid.', 'shopos-core' ),
			),
			'hybrid_threshold'  => array(
				'label'       => __( 'Hybrid threshold (pages)', 'shopos-core' ),
				'type'        => 'number',
				'default'     => 2,
				'description' => __( 'In Hybrid mode, the number of pages to auto-load before switching to the Load-more button. Ignored unless trigger mode is set to Hybrid.', 'shopos-core' ),
			),
			'container_selector' => array(
				'label'       => __( 'Container selector override', 'shopos-core' ),
				'type'        => 'text',
				'default'     => '',
			...
			'shimmer_base_color'      => array(
				...
				'type'        => 'color',
				'default'     => '#eceff3',
				'description' => __( 'Base color of the skeleton-card shimmer gradient.', 'shopos-core' ),
			),
			'shimmer_highlight_color' => array(
				'label'       => __( 'Skeleton shimmer — highlight color', 'shopos-core' ),
				'type'        => 'color',
				'default'     => '#f6f8fb',
				'description' => __( 'Highlight color that sweeps across the shimmer gradient.', 'shopos-core' ),
			),
			'shimmer_duration_ms'     => array(
				'label'       => __( 'Skeleton shimmer — duration (ms)', 'shopos-core' ),
				'type'        => 'number',
				'default'     => 1400,
				'description' => __( 'How long one shimmer sweep takes, in milliseconds.', 'shopos-core' ),
			),
			'fade_duration_ms'        => array(
				'label'       => __( 'New product fade-in — duration (ms)', 'shopos-core' ),
				'type'        => 'number',
				'default'     => 550,
				'description' => __( 'Duration of the appear-animation on newly-loaded product cards.', 'shopos-core' ),
			),
			'fade_transform_px'       => array(
				'label'       => __( 'New product fade-in — translate distance (px)', 'shopos-core' ),
				'type'        => 'number',
				'default'     => 18,
				'description' => __( 'Vertical offset (px) that newly-loaded product cards rise from during fade-in.', 'shopos-core' ),
			),
		);
	}
	...
	public function enqueue() {
		...
		$handle            = 'shopos-core-infinite-scroll';
		$deprecated_handle = 'shopos-infinite-scroll';
		...
		if ( ! wp_style_is( $deprecated_handle, 'registered' ) ) {
			wp_register_style( $deprecated_handle, false, array( $handle ), SHOPOS_CORE_VERSION );
		}
		if ( ! wp_script_is( $deprecated_handle, 'registered' ) ) {
			wp_register_script( $deprecated_handle, false, array( $handle ), SHOPOS_CORE_VERSION, true );
		}

		wp_localize_script(
			$handle,
			'ShopOSInfiniteScroll',
			$this->localized_payload()
		);
	}

	/**
	 * Build the ShopOSInfiniteScroll JS payload.
	 *
	...
	public function localized_payload() {
		return array(
			...
			'rootMargin'          => (string) apply_filters( 'shopos_core/infinite_scroll/root_margin', '800px 0px' ),
			'endMessage'          => (string) $this->get_option( 'end_message', __( 'You have reached the end.', 'shopos-core' ) ),
			'errorMessage'        => __( 'Could not load more.', 'shopos-core' ),
			'loadMoreLabel'       => __( 'Load more', 'shopos-core' ),
			/* translators: %d = number of products just loaded. Used for screen-reader aria-live announcement. */
			'announceTemplate'    => __( 'Loaded %d more products.', 'shopos-core' ),
	...
	public function should_render_wrapper() {
		...
		$resolved = apply_filters( 'shopos_core/infinite_scroll/should_render_wrapper', $resolved );
	...
	public function render_grid_wrapper_open() {
		...
		do_action( 'shopos_core/infinite_scroll/before_render' );
		echo '<div class="shopos-is-wrapper">';
	}
	...
	public function render_grid_wrapper_close() {
		...
		echo '</div>';
		do_action( 'shopos_core/infinite_scroll/after_render' );
	 ...
	 * `shopos_core/infinite_scroll/selector` filter (final word per the
	 * shopos-core setting+filter convention), and normalizes to an array
	...
	public function resolve_container_selector() {
		...
		$resolved = apply_filters( 'shopos_core/infinite_scroll/selector', $setting );

 ...
 *
 * Owns the filesystem layout (uploads/shopos-product-feed/) and the actual
...
final class Generator {
	...
	const OPT_LAST_GEN = 'shopos_core_productfeed_last_generated';
	...
	/**
	 * Feed directory (uploads/shopos-product-feed).
	 *
	 * @return string
	 */
	public function feed_dir() {
		$up = wp_upload_dir();
		return trailingslashit( $up['basedir'] ) . 'shopos-product-feed';
	}
	...
	public function feed_url() {
		$up = wp_upload_dir();
		return trailingslashit( $up['baseurl'] ) . 'shopos-product-feed/products.xml.gz';
	}
	...
	public function generate() {
		...
		try {
			 ...
			 */
			do_action( 'shopos_core/product_feed/after_generate', $this->feed_file(), $elapsed );
		} catch ( \Throwable $e ) {
	...
	private function write_feed( $tmp_file ) {
		...
		while ( true ) {
			...
			foreach ( $ids as $pid ) {
				 ...
				 */
				$xml = (string) apply_filters( 'shopos_core/product_feed/item', $this->product_xml( $product ), $product );
				$write( $xml );

...
- New Elementor controls (gated on `shopos_core_sliders_advanced_controls_enabled`, default off): **Autoplay**, **Autoplay delay** (clamped 1000–15000 ms), **Loop** (autoplay-wrap-to-start; drag-past-end-wraps deliberately out of scope), and **Indicator** (`progress` / `dots` / `none`). The legacy **Show progress bar** switcher stays in place as a back-compat alias — when `Indicator` is unset on a pre-existing widget, render falls through to the legacy value, so flipping the flag on doesn't move any saved widget away from its current behavior.
...
- Runtime is now shared with the new **ProductSlider** widget — both render the same `.cs[data-cs-snap]` skeleton, so a single drag / momentum / progress engine drives both. Two surgical edits to `category-slider.js`:
  ...
  - The Elementor `frontend/element_ready` action now subscribes both `shopos_category_slider.default` and `shopos_product_slider.default`, so the editor preview re-init fires for either widget. CategorySlider rendering is unchanged.
...
- Registers `shopos_category_slider` Elementor widget under the WooCommerce panel.

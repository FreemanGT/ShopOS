 ...
 *
 * The legacy `ShopOS_VS_Settings` static helpers (bool / max_visible /
 ...
 *
 * As of Wave 2.2 / 4g (1.11.45) the settings are owned by the ShopOS →
 * Variation Swatches admin page and stored under
 * `shopos_core_variation_swatches_*`. This reader prefers that key and falls
 * back to the legacy `shopos_vs_*` key (which is never deleted — per the
 ...
 * `shopos_core_variation_swatches_settings_hub_enabled` flag (flag OFF read
...
final class Settings_Reader {
	...
	// Intentionally non-`shopos_*` to stay out of baseline-options-declared.txt;
	// this is an in-memory marker, never persisted.
	const SENTINEL = '__FR_NOT_SET__';

	const LEGACY_PREFIX = 'shopos_vs_';

	const NEW_PREFIX = 'shopos_core_variation_swatches_';
	 ...
	 * (`ShopOS_VS_Settings::bool()`) compares against the strings 'yes'/'no'.
	 *
	 * **Maintenance contract**: when adding a new `'checkbox'`-typed entry to
	 * {@see \ShopOS\Core\Modules\VariationSwatches\Module::settings_schema()},
	 ...
	 * `shopos_vs_shop_excluded_categories` (string-stored, array-consumed) —
	...
	private const CHECKBOX_KEYS = array(
		'shopos_vs_shop_enabled',
		'shopos_vs_shop_show_price',
		'shopos_vs_shop_apply_shop',
		'shopos_vs_shop_apply_category',
		'shopos_vs_shop_apply_tag',
		...
		'shopos_vs_shop_hide_selected',
		'shopos_vs_shop_names_price_only',
	);

	/**
	 * Read a setting: new key first, legacy key as fallback, caller default last.
	 *
	 * @param string $legacy_key Full legacy option key (e.g. shopos_vs_shop_enabled).
	...
	private static function normalize_new_value_for_legacy_reader( $legacy_key, $value ) {
		...
		if ( 'shopos_vs_shop_excluded_categories' === $legacy_key && is_string( $value ) ) {
			if ( '' === trim( $value ) ) {
				return array();
			}
	...
	/**
	 * Map a legacy shopos_vs_* key to its shopos_core_variation_swatches_*

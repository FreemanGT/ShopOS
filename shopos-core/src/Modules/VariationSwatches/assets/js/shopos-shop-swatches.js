/**
 * ShopOS Variation Swatches — shop / archive compact picker (1.6.0).
 *
 ...
 *   - Add-to-cart → POST to wc-ajax=shopos_shop_add_to_cart with nonce.
...
(function () {
	...
	var i18n = (window.ShopOSShopVS && window.ShopOSShopVS.i18n) || {};
	var AJAX_URL = (window.ShopOSShopVS && window.ShopOSShopVS.ajaxUrl) || '';
	var CART_URL = (window.ShopOSShopVS && window.ShopOSShopVS.cartUrl) || '';
	...
	var CARD_IMAGE_SELECTOR = (window.ShopOSShopVS && window.ShopOSShopVS.cardImageSelector) || '';
	...
	function getPickerData(picker) {
		if (!picker._shoposData) {
			try {
				var raw = picker.getAttribute('data-variations');
				picker._shoposData = raw ? JSON.parse(raw) : [];
			} catch (e) {
				picker._shoposData = [];
			}
		}
		return picker._shoposData;
	}

	function getSelections(picker) {
		var out = {};
		qsa('.shopos-shop-pick__attr', picker).forEach(function (row) {
			var name = row.getAttribute('data-attribute-name') || '';
			if (!name) return;
			var sel = qs('.shopos-shop-pick__opt.is-selected', row);
	...
	function computeStates(picker, targetAttr) {
		...
		var result = {};
		qsa('.shopos-shop-pick__opt', targetRow).forEach(function (btn) {
	...
	function refreshAvailability(picker) {
		qsa('.shopos-shop-pick__attr', picker).forEach(function (row) {
			...
			qsa('.shopos-shop-pick__opt', row).forEach(function (btn) {
	...
	function refreshHeadLabels(picker) {
		qsa('.shopos-shop-pick__attr', picker).forEach(function (row) {
			var selBtn   = qs('.shopos-shop-pick__opt.is-selected', row);
			var headSpan = qs('.shopos-shop-pick__attr-selected', row);
			if (!headSpan) return;
	...
	function refreshAddState(picker) {
		var btn = qs('.shopos-shop-pick__add', picker);
		if (!btn) return;
	...
	function refreshPrice(picker) {
		var priceEl  = qs('.shopos-shop-pick__price-value', picker);
		var priceRow = qs('.shopos-shop-pick__price', picker);
		var prefix   = qs('.shopos-shop-pick__price-prefix', picker);
		if (!priceEl) return;
		...
		if (variation && variation.price_html) {
			...
			if (!picker._shoposOriginalPrice) {
				// Stash the server-rendered "from" price so we can restore it
				// when the user changes a selection back to an unresolved state.
				picker._shoposOriginalPrice = priceEl.innerHTML;
			} else {
				priceEl.innerHTML = picker._shoposOriginalPrice;
			}
	...
	function refreshCardImage(picker) {
		...
		if (!picker._shoposOriginalCardImage) {
			picker._shoposOriginalCardImage = {
		...
		var orig = picker._shoposOriginalCardImage;
	...
	function refreshAll(picker) {
		...
		if (picker.classList && picker.classList.contains('shopos-shop-pick--simple')) {
			return;
		}
	...
	function refreshOverflow(picker) {
		qsa('.shopos-shop-pick__attr', picker).forEach(function (row) {
			var opts = qs('.shopos-shop-pick__opts', row);
			if (!opts) return;

			var moreBtn = qs('.shopos-shop-pick__more', opts);
			var rawChips = qsa('.shopos-shop-pick__opt', opts);
			...
			if (moreBtn) {
				...
				var lbl = qs('.shopos-shop-pick__more-label', moreBtn);
				if (lbl) {
	 ...
	 *
	 * Visual style: matches the PDP `.shopos-toast` (dark pill, bottom-left,
	...
	function ensureToastStack() {
		...
		_toastStack.className = 'shopos-shop-toast-stack';
	...
	function showToast(text, type, ttl) {
		...
		toast.className = 'shopos-shop-toast shopos-shop-toast--' + type;
		...
		if (type === 'success') {
			...
			check.className = 'shopos-shop-toast__check';
			check.setAttribute('aria-hidden', 'true');
			toast.appendChild(check);
		} else if (type === 'error') {
			var bang = document.createElement('span');
			bang.className = 'shopos-shop-toast__bang';
	...
	function dismissToast(toast) {
		...
		} else if (toast._shoposTimer) {
			clearTimeout(toast._shoposTimer);
			toast._shoposTimer = null;
		}
	...
	document.addEventListener('click', function (e) {
		...
		var optBtn = closest(target, '.shopos-shop-pick__opt');
		if (optBtn && closest(optBtn, '.shopos-shop-pick')) {
			e.preventDefault();
			e.stopPropagation();
			...
			var row    = closest(optBtn, '.shopos-shop-pick__attr');
			var picker = closest(optBtn, '.shopos-shop-pick');
			if (!row || !picker) return;

			var alreadySelected = optBtn.classList.contains('is-selected');
			qsa('.shopos-shop-pick__opt', row).forEach(function (other) {
		...
		// --- +N reveal -------------------------------------------------
		var moreBtn = closest(target, '.shopos-shop-pick__more');
		if (moreBtn && closest(moreBtn, '.shopos-shop-pick')) {
			e.preventDefault();
			e.stopPropagation();
			var row2   = closest(moreBtn, '.shopos-shop-pick__attr');
			if (!row2) return;
			...
			var optsContainer = qs('.shopos-shop-pick__opts', row2);
			if (optsContainer) {
				optsContainer.classList.toggle('is-expanded', !expanded);
			}
			qsa('.shopos-shop-pick__opt.is-overflow', row2).forEach(function (o) {
				if (expanded) {
					o.hidden = true;
		...
		var qtyBtn = closest(target, '.shopos-shop-pick__qty-btn');
		if (qtyBtn && closest(qtyBtn, '.shopos-shop-pick--simple')) {
			e.preventDefault();
			e.stopPropagation();
			var pickerQ  = closest(qtyBtn, '.shopos-shop-pick');
			if (!pickerQ) return;
			var qtyInput = qs('.shopos-shop-pick__qty', pickerQ);
			if (!qtyInput) return;
			...
			var next = qtyBtn.classList.contains('shopos-shop-pick__qty-btn--plus')
				? val + step
				: val - step;
		...
		// --- Add to cart ----------------------------------------------
		var addBtn = closest(target, '.shopos-shop-pick__add');
		if (addBtn && closest(addBtn, '.shopos-shop-pick')) {
			e.preventDefault();
			e.stopPropagation();
			if (addBtn.disabled || addBtn.classList.contains('is-busy')) return;
			var picker2 = closest(addBtn, '.shopos-shop-pick');
			if (!picker2) return;
	...
	function submitAddToCart(picker, addBtn) {
		...
		var isSimple = picker.classList.contains('shopos-shop-pick--simple');
		...
		if (isSimple) {
			...
			var qtyInput = qs('.shopos-shop-pick__qty', picker);
			var qty      = 1;
			if (qtyInput) {
	...
	function initAllPickers(root) {
		qsa('.shopos-shop-pick', root || document).forEach(function (p) {
			refreshAll(p);
		});
	}
	...
	window.addEventListener('resize', function () {
		...
		resizeRaf = requestAnimationFrame(function () {
			resizeRaf = 0;
			qsa('.shopos-shop-pick').forEach(function (p) { refreshOverflow(p); });
		});
	});
		...
		var mo = new MutationObserver(function (mutations) {
			for (var i = 0; i < mutations.length; i++) {
				...
				for (var j = 0; j < m.addedNodes.length; j++) {
					...
					if (n.classList && n.classList.contains('shopos-shop-pick')) {
						refreshAll(n);
					} else {
						qsa('.shopos-shop-pick', n).forEach(function (p) { refreshAll(p); });
					}
				}
			}
		});

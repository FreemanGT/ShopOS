 ...
 *   - Feature-flagged: controlled entirely by ShopOS_VS_Settings. If the
...
if ( ! class_exists( 'ShopOS_VS_Archive' ) ) :

class ShopOS_VS_Archive {
	...
	public function register(): void {
		...
		add_action( 'wc_ajax_shopos_shop_add_to_cart',         [ $this, 'ajax_add_to_cart' ] );
		add_action( 'wc_ajax_nopriv_shopos_shop_add_to_cart',  [ $this, 'ajax_add_to_cart' ] );
	...
	public function maybe_swap_loop_price_action(): void {
		...
		if ( ShopOS_VS_Settings::bool( ShopOS_VS_Settings::OPT_SHOP_NAMES_PRICE_ONLY, 'no' ) ) {
			return; // names + price only: the picker is suppressed, so leave WC's default loop price so cards still show a price.
		}
		if ( ! ShopOS_VS_Settings::bool( ShopOS_VS_Settings::OPT_SHOW_PRICE, 'yes' ) ) {
	...
	public function render_loop_price_or_skip(): void {
		...
		$picker_owns_price = ( $product->is_type( 'variable' ) || $product->is_type( 'simple' ) )
			&& ShopOS_VS_Settings::should_apply_on_current_archive()
	...
	private function is_product_excluded( WC_Product $product ): bool {
		$excluded = ShopOS_VS_Settings::excluded_category_ids();
		if ( empty( $excluded ) ) {
			return false;
		}
	...
	public function maybe_replace_loop_link( $html, $product, $args ) {
		...
		// quantity to add, neither maps cleanly onto the ShopOS card.
		if ( ! $product->is_type( 'variable' ) && ! $product->is_type( 'simple' ) ) {
			return $html;
		}
		if ( ! ShopOS_VS_Settings::should_apply_on_current_archive() ) {
			return $html;
		}
	...
	private function prepare_simple_product_data( WC_Product $product ): array {
		...
		$data = [
			...
			'ajax_url'       => \WC_AJAX::get_endpoint( 'shopos_shop_add_to_cart' ),
			'nonce'          => wp_create_nonce( 'shopos_vs_shop_' . $pid ),
		];
	...
	private function render_simple_template( WC_Product $product, array $prepared ): string {
		$template_vars = [
			...
			'show_price' => ShopOS_VS_Settings::bool( ShopOS_VS_Settings::OPT_SHOW_PRICE, 'yes' ),
		];
		...
		wc_get_template(
			...
			$template_vars,
			'shopos-variation-swatches/',
			SHOPOS_VS_DIR . 'templates/'
		);
	...
	private function prepare_product_data( WC_Product $product ): array {
		...
		if ( is_array( $cached ) && isset( $cached['pid'] ) ) {
			$cached['nonce']    = wp_create_nonce( 'shopos_vs_shop_' . $pid );
			$cached['ajax_url'] = \WC_AJAX::get_endpoint( 'shopos_shop_add_to_cart' );
		...
		if ( class_exists( 'ShopOS_VS_Settings' )
			&& ShopOS_VS_Settings::bool( ShopOS_VS_Settings::OPT_SHOP_HIDE_OOS, 'yes' )
		) {
			list( $attributes, $available_variations ) = ShopOS_VS_Plugin::filter_in_stock_only(
				$attributes,
		...
		$image_swatches_on = \ShopOS\Core\Core\Feature_Flags::is_enabled( 'variation_swatches', 'image_swatches' );
		$tooltip_on        = \ShopOS\Core\Core\Feature_Flags::is_enabled( 'variation_swatches', 'tooltip' );
		...
		foreach ( $attributes as $attribute_name => $options ) {
			...
			$options    = ShopOS_VS_Plugin::reorder_options_to_match_terms( (int) $pid, $taxonomy, (array) $options );
			$is_color   = $taxonomy && ShopOS_VS_Plugin::attribute_is_color( $taxonomy );
			$has_images = $image_swatches_on && $taxonomy && ShopOS_VS_Plugin::attribute_has_images( $taxonomy );
			...
			foreach ( $options as $option ) {
				...
				if ( $taxonomy ) {
					...
					if ( $term && ! is_wp_error( $term ) ) {
						$name = $term->name;
						$hex  = \ShopOS\Core\Modules\VariationSwatches\Color_Sampler::resolve_term_color( (int) $term->term_id, (int) $pid );
						if ( $image_swatches_on ) {
							$img = ShopOS_VS_Plugin::term_image_url( (int) $term->term_id, 'thumbnail' );
						}
						if ( $tooltip_on ) {
							$tt = ShopOS_VS_Plugin::term_tooltip_text( (int) $term->term_id, $name );
						}
					}
				}
				$item = [
			...
			$honor_defaults = ! ( class_exists( 'ShopOS_VS_Settings' )
				&& ShopOS_VS_Settings::bool( ShopOS_VS_Settings::OPT_SHOP_NO_PRESELECT, 'yes' ) );
			...
			$entry_attr = [
				'name'     => $input_name,
				'label'    => ShopOS_VS_Plugin::resolve_attribute_label( $attribute_name, $product ),
				'is_color' => $is_color,
				'selected' => $selected,
		...
		$swap_card_image = \ShopOS\Core\Core\Feature_Flags::is_enabled( 'variation_swatches', 'card_image_swap' );
	 ...
	 * appended (passing through the `shopos_core/variation_swatches/card_image_payload`
	 * filter). When false, the entry has only id / attrs / in_stock / is_purchasable
	 * / price_html — byte-identical to pre-1.11.23.
	 *
	 * @internal Used by ShopOS_VS_Archive only; signature may change without notice.
	 *
	...
	private function prepared_transient_key( int $pid ): string {
		...
		$swap_flag     = \ShopOS\Core\Core\Feature_Flags::is_enabled( 'variation_swatches', 'card_image_swap' ) ? '1' : '0';
		...
		// image_url fields.
		$image_flag    = \ShopOS\Core\Core\Feature_Flags::is_enabled( 'variation_swatches', 'image_swatches' ) ? '1' : '0';
		// Wave 2.2 / 4c (1.11.25) — same trick for the tooltip flag.
		$tooltip_flag  = \ShopOS\Core\Core\Feature_Flags::is_enabled( 'variation_swatches', 'tooltip' ) ? '1' : '0';
		$signature     = implode( '|', [ $pid, $wc_ver, $display, $curr, $curr_pos, $dec_sep, $thou_sep, $decimals, $swap_flag, $image_flag, $tooltip_flag ] );
		return 'shopos_vs_pd_' . md5( $signature );
	}
	...
	private function render_template( WC_Product $product, array $prepared ): string {
		$template_vars = [
			'product'           => $product,
			'prepared'          => $prepared,
			'max_visible'       => ShopOS_VS_Settings::max_visible(),
			...
			'show_price'        => ShopOS_VS_Settings::bool( ShopOS_VS_Settings::OPT_SHOW_PRICE, 'yes' ),
			'hide_attr_labels'  => ShopOS_VS_Settings::bool( ShopOS_VS_Settings::OPT_SHOP_HIDE_ATTR_LABELS, 'yes' ),
			'hide_selected'     => ShopOS_VS_Settings::bool( ShopOS_VS_Settings::OPT_SHOP_HIDE_SELECTED, 'yes' ),
		];
	...
	public function register_assets(): void {
		...
		$fs_base  = dirname( rtrim( SHOPOS_VS_DIR, '/' ) ) . '/assets/';
		$url_base = SHOPOS_VS_URL . 'assets/';
		$pick     = array( '\\ShopOS\\Core\\Core\\Module_Base', 'pick_min_url' );

		wp_register_style(
			'shopos-vs-shop',
			call_user_func( $pick, $fs_base, $url_base, 'css/shopos-shop-swatches.css' ),
			[],
			SHOPOS_VS_VERSION
		);
		...
		wp_register_script(
			'shopos-vs-shop',
			call_user_func( $pick, $fs_base, $url_base, 'js/shopos-shop-swatches.js' ),
			[],
			SHOPOS_VS_VERSION,
			true
		);
		...
		$card_image_selector = '';
		if ( \ShopOS\Core\Core\Feature_Flags::is_enabled( 'variation_swatches', 'card_image_swap' ) ) {
			/**
			...
			$card_image_selector = (string) apply_filters(
				'shopos_core/variation_swatches/card_image_selector',
				'.woocommerce-loop-product__link img, .product-thumb img'
			);
		}

		wp_localize_script( 'shopos-vs-shop', 'ShopOSShopVS', [
			'ajaxUrl'            => \WC_AJAX::get_endpoint( 'shopos_shop_add_to_cart' ),
			'cartUrl'            => function_exists( 'wc_get_cart_url' ) ? wc_get_cart_url() : '',
			'cardImageSelector'  => $card_image_selector,
			'i18n'               => [
				'choose'       => \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'choose_option' ),
				'notAvailable' => \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'not_available' ),
				'addedToCart'  => \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'added_to_cart' ),
				'addToCart'    => \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'add_to_cart' ),
				'selectOpts'   => \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'select_options' ),
				'showMore'     => '+%d',
				'showLess'     => '−',
				'fromPrice'    => \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'from_price' ),
				'oos'          => \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'out_of_stock' ),
				'unavailable'  => \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'unavailable' ),
				'errorGeneric' => \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'error_generic' ),
				// Toast (1.6.4) — aria labels.
				'close'        => \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'close' ),
				'notices'      => \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'notices' ),
			],
		] );
		...
		if ( ShopOS_VS_Settings::should_apply_on_current_archive() ) {
			wp_enqueue_style( 'shopos-vs-shop' );
			wp_enqueue_script( 'shopos-vs-shop' );
		}
	}
	...
	public function ajax_add_to_cart(): void {
		...
		if ( ! empty( $_POST['_hp'] ) ) {
			wp_send_json_error( [
				'message' => __( 'Session expired. Please refresh the page.', 'shopos-core' ),
			], 400 );
		}
		...
		if ( ! $product_id || ! wp_verify_nonce( $nonce, 'shopos_vs_shop_' . $product_id ) ) {
			wp_send_json_error( [
				'message' => __( 'Session expired. Please refresh the page.', 'shopos-core' ),
			], 400 );
		}
		...
		if ( is_wp_error( $gate ) ) {
			wp_send_json_error( [
				'message' => $gate->get_error_message() ?: __( 'This product is not available.', 'shopos-core' ),
			], 403 );
		}
		...
		if ( ! $product instanceof WC_Product ) {
			wp_send_json_error( [
				'message' => __( 'Product not available.', 'shopos-core' ),
			], 404 );
		}
		...
		if ( ! $is_simple_pick ) {
			...
			if ( ! $variation_id ) {
				wp_send_json_error( [
					'message' => __( 'This combination is not available.', 'shopos-core' ),
				], 400 );
			}
		} else {
			$variation_id = 0;
		...
		if ( ! $passed ) {
			...
			wp_send_json_error( [
				'message' => $msg ?: __( 'This combination is not available.', 'shopos-core' ),
			], 400 );
		}
		...
		if ( ! $cart_item_key ) {
			...
			wp_send_json_error( [
				'message' => $msg ?: __( 'Could not add to cart.', 'shopos-core' ),
			], 400 );
		}
...
endif; // class_exists ShopOS_VS_Archive

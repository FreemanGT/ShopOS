...
if ( ! class_exists( 'ShopOS_VS_Plugin' ) ) :

final class ShopOS_VS_Plugin {

	/** @var ShopOS_VS_Plugin|null */
	private static $instance = null;

	/** @var ShopOS_VS_Admin */
	public $admin;

	/** @var ShopOS_VS_Frontend */
	public $frontend;

	/** @var ShopOS_VS_Ajax */
	public $ajax;

	/** @var ShopOS_VS_Settings */
	public $settings;

	/** @var ShopOS_VS_Archive */
	public $archive;

	public static function instance(): ShopOS_VS_Plugin {
	...
	public function boot(): void {
		// Text-domain (`shopos-core`) is already loaded once by Plugin::load_textdomain();
		...
		$this->admin    = new ShopOS_VS_Admin();
		$this->frontend = new ShopOS_VS_Frontend();
		$this->ajax     = new ShopOS_VS_Ajax();
		$this->settings = new ShopOS_VS_Settings();
		$this->archive  = new ShopOS_VS_Archive();
	...
	private static function maybe_run_migrations(): void {
		...
		$flag_key = 'shopos_vs_migrated_1_6_6';
		...
		$existing = \ShopOS\Core\Modules\VariationSwatches\Settings_Reader::get( ShopOS_VS_Settings::OPT_PDP_HIDE_OOS, null );
		if ( 'yes' === $existing ) {
			update_option( ShopOS_VS_Settings::OPT_PDP_HIDE_OOS, 'no' );
		}
	 ...
	 * `shopos_core_variation_swatches_*` rather than the legacy `shopos_*`
	 ...
	 * copies shopos_vs_* values into shopos_core_variation_swatches_* keys), so
	...
	public static function image_meta_key(): string {
		return 'shopos_core_variation_swatches_term_image_id';
	}
	 ...
	 *
	 * Filterable via `shopos_core/variation_swatches/term_image_url` so sites
	...
	public static function term_image_url( int $term_id, string $size = 'thumbnail' ): string {
		 ...
		 */
		return (string) apply_filters( 'shopos_core/variation_swatches/term_image_url', $url, $term_id, $size );
	}
	 ...
	 * image_meta_key() — keeps new term-meta keys in `shopos_core_*` so the
	 * legacy `shopos_*` namespace doesn't grow further.
	 */
	public static function tooltip_meta_key(): string {
		return 'shopos_core_variation_swatches_term_tooltip_text';
	}
...
endif; // class_exists ShopOS_VS_Plugin

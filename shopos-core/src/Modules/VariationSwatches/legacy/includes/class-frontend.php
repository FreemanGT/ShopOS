...
if ( ! class_exists( 'ShopOS_VS_Frontend' ) ) :

class ShopOS_VS_Frontend {
	...
	public function register(): void {
		...
		// Register shortcode.
		add_shortcode( 'shopos_buy_box', [ $this, 'shortcode' ] );
	 ...
	 * link with no quantity, so neither maps cleanly onto the ShopOS
	 * buy box).
	 */
	public function swap_variable_add_to_cart(): void {
		...
		// products — both buy-box templates emit their own `.shopos-pdp-price`
	 ...
	 * `.shopos-pdp-price` line via our buy-box templates, so WC's price
	 ...
	 *   [shopos_buy_box]            — uses the current product on a single-product page
	 *   [shopos_buy_box id="123"]   — forces a specific product
	 */
	public function shortcode( $atts = [] ): string {
		$atts = shortcode_atts( [
			'id' => 0,
		], is_array( $atts ) ? $atts : [], 'shopos_buy_box' );
	...
	/**
	 * Render the ShopOS buy box for a simple product. Uses the same
	 * `.shopos-buy-box` shell as the variable template (so all existing
	...
	private function render_variable( WC_Product $product ): void {
		...
		if ( class_exists( 'ShopOS_VS_Settings' )
			&& ShopOS_VS_Settings::bool( ShopOS_VS_Settings::OPT_PDP_HIDE_OOS, 'no' )
		) {
		...
		// Allow theme overrides via /woocommerce/shopos-variation-swatches/variation-buy-box.php
		wc_get_template(
	...
	public function register_assets(): void {
		$fs_base  = SHOPOS_VS_DIR . '../assets/';
		$url_base = SHOPOS_VS_URL . 'assets/';
		// SHOPOS_VS_DIR ends with 'legacy/'; assets live one dir up.
		$fs_base  = dirname( rtrim( SHOPOS_VS_DIR, '/' ) ) . '/assets/';
		$pick     = array( '\\ShopOS\\Core\\Core\\Module_Base', 'pick_min_url' );

		wp_register_style(
			'shopos-core',
			call_user_func( $pick, $fs_base, $url_base, 'css/shopos-swatches.css' ),
			[],
			SHOPOS_VS_VERSION
		);
		...
		wp_register_script(
			'shopos-core',
			call_user_func( $pick, $fs_base, $url_base, 'js/shopos-swatches.js' ),
		...
		$checkout_url = (string) apply_filters( 'shopos_vs_checkout_url', $checkout_url );

		wp_localize_script( 'shopos-core', 'ShopOSVS', [
		...
		// only targets `.shopos-buy-box` selectors, so pages without the
		...
		if ( ! is_admin() ) {
			...
			wp_enqueue_style( 'shopos-core' );
			wp_enqueue_script( 'shopos-core' );
		}
	}

	/**
	 * The locale-aware i18n strings handed to the swatches JS via
	 * `ShopOSVS.i18n`. Pure (no WP state) so it is unit-testable; the
	...
	public static function i18n_payload(): array {
		$labels = '\\ShopOS\\Core\\Modules\\VariationSwatches\\Labels';
...
endif; // class_exists ShopOS_VS_Frontend

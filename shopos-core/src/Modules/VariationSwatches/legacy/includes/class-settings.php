 ...
 *
 * As of Wave 2.2 / 4g (shopos-core 1.11.45) the editable settings UI moved
 * to **ShopOS → Variation Swatches** (the Settings_Hub page, stored under
 * `shopos_core_variation_swatches_*` and read via {@see
 * \ShopOS\Core\Modules\VariationSwatches\Settings_Reader}). Through 1.23.1 a
 ...
 * old `?section=shopos_vs_shop_pick` URL kept resolving.
 *
 ...
 * `shopos_vs_*` option keys are never deleted (§4.5 zero-downtime decision).
 *
...
class ShopOS_VS_Settings {

	/** Individual option keys, one per field. */
	public const OPT_ENABLED             = 'shopos_vs_shop_enabled';
	public const OPT_MAX_VISIBLE         = 'shopos_vs_shop_max_visible';
	public const OPT_SHOW_PRICE          = 'shopos_vs_shop_show_price';
	public const OPT_APPLY_SHOP          = 'shopos_vs_shop_apply_shop';
	public const OPT_APPLY_CATEGORY      = 'shopos_vs_shop_apply_category';
	public const OPT_APPLY_TAG           = 'shopos_vs_shop_apply_tag';
	public const OPT_APPLY_SEARCH        = 'shopos_vs_shop_apply_search';
	public const OPT_APPLY_RELATED       = 'shopos_vs_shop_apply_related';
	public const OPT_EXCLUDED_CATEGORIES = 'shopos_vs_shop_excluded_categories';
	...
	public const OPT_SHOP_NO_PRESELECT   = 'shopos_vs_shop_no_preselect';

	/** Archive / shop-grid: hide the attribute label row e.g. "Size:" (1.7.8). */
	public const OPT_SHOP_HIDE_ATTR_LABELS = 'shopos_vs_shop_hide_attr_labels';

	/** Archive / shop-grid: hide the "selected option" text row (1.7.9). */
	public const OPT_SHOP_HIDE_SELECTED    = 'shopos_vs_shop_hide_selected';

	/** Archive / shop-grid: show name + price only, no buy UI (1.23.2). */
	public const OPT_SHOP_NAMES_PRICE_ONLY = 'shopos_vs_shop_names_price_only';

	/**
	 * Boot hook, called from ShopOS_VS_Plugin. As of 1.23.2 the WooCommerce
	...
	public static function bool( string $option_key, string $default = 'yes' ): bool {
		...
		return 'yes' === \ShopOS\Core\Modules\VariationSwatches\Settings_Reader::get( $option_key, $default );
	}

	public static function max_visible(): int {
		$v = absint( \ShopOS\Core\Modules\VariationSwatches\Settings_Reader::get( self::OPT_MAX_VISIBLE, 5 ) );
		if ( $v < 1 )  $v = 1;
		if ( $v > 50 ) $v = 50;
		return $v;
	}
	...
	public static function excluded_category_ids(): array {
		$raw = \ShopOS\Core\Modules\VariationSwatches\Settings_Reader::get( self::OPT_EXCLUDED_CATEGORIES, [] );
		if ( ! is_array( $raw ) ) {
			return [];
		}
...
endif; // class_exists ShopOS_VS_Settings

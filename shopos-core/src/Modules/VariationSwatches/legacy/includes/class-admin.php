...
if ( ! class_exists( 'ShopOS_VS_Admin' ) ) :

class ShopOS_VS_Admin {
	...
	public function enqueue_color_picker( string $hook ): void {
		...
		// to pre-1.11.24.
		if ( ! \ShopOS\Core\Core\Feature_Flags::is_enabled( 'variation_swatches', 'image_swatches' ) ) {
			return;
		}
	...
	/**
	 * Inline JS that wires the .shopos-image-field UI to wp.media().
	 *
...
jQuery(function($) {
	...
	$(document).on('click', '.shopos-image-field-pick', function(e) {
		e.preventDefault();
		var $btn  = $(this);
		var $row  = $btn.closest('.shopos-image-field-wrap');
		var $hid  = $row.find('.shopos-image-field-id');
		var $prev = $row.find('.shopos-image-field-preview');
		...
		frame.on('select', function() {
			...
			$row.find('.shopos-image-field-clear').show();
		});
		frame.open();
	});
	$(document).on('click', '.shopos-image-field-clear', function(e) {
		e.preventDefault();
		var $row  = $(this).closest('.shopos-image-field-wrap');
		$row.find('.shopos-image-field-id').val('0');
		$row.find('.shopos-image-field-preview').attr('src', '').hide();
		$(this).hide();
	});
});
JS;
	...
	public function render_add_field(): void {
		?>
		<div class="form-field term-shopos-color-wrap">
			<label for="shopos_swatch_color"><?php esc_html_e( 'Swatch color', 'shopos-core' ); ?></label>
			<input type="text" name="shopos_swatch_color" id="shopos_swatch_color" value="" class="shopos-color-field" />
			<p class="description">
				<?php esc_html_e( 'Set a color here and this term will render as a color circle on the product page. Leave empty to render as a text button.', 'shopos-core' ); ?>
			</p>
		</div>
	...
	public function render_edit_field( WP_Term $term, string $taxonomy ): void {
		$value = ShopOS_VS_Plugin::term_color( $term->term_id );
		?>
		<tr class="form-field term-shopos-color-wrap">
			<th scope="row"><label for="shopos_swatch_color"><?php esc_html_e( 'Swatch color', 'shopos-core' ); ?></label></th>
			<td>
				<input type="text" name="shopos_swatch_color" id="shopos_swatch_color" value="<?php echo esc_attr( $value ); ?>" class="shopos-color-field" />
				<p class="description">
					<?php esc_html_e( 'Set a color here and this term will render as a color circle on the product page. Leave empty to render as a text button.', 'shopos-core' ); ?>
				</p>
			</td>
	...
	private function render_image_field( int $term_id ): void {
		...
		$attachment_id = $term_id > 0 ? ShopOS_VS_Plugin::term_image_id( $term_id ) : 0;
		$preview_url   = $term_id > 0 ? ShopOS_VS_Plugin::term_image_url( $term_id, 'thumbnail' ) : '';
		...
		if ( $is_edit_screen ) :
			?>
			<tr class="form-field term-shopos-image-wrap">
				<th scope="row"><label for="shopos_swatch_image_id"><?php esc_html_e( 'Swatch image', 'shopos-core' ); ?></label></th>
				<td class="shopos-image-field-wrap">
			...
			?>
			<div class="form-field term-shopos-image-wrap shopos-image-field-wrap">
				<label for="shopos_swatch_image_id"><?php esc_html_e( 'Swatch image', 'shopos-core' ); ?></label>
				<?php $this->render_image_field_inner( $attachment_id, $preview_url, $has_image ); ?>
			</div>
			<?php
		endif;
	...
	private function render_tooltip_field( int $term_id ): void {
		if ( ! \ShopOS\Core\Core\Feature_Flags::is_enabled( 'variation_swatches', 'tooltip' ) ) {
			return;
		}

		$value          = $term_id > 0 ? ShopOS_VS_Plugin::term_tooltip_text( $term_id ) : '';
		$is_edit_screen = $term_id > 0;

		if ( $is_edit_screen ) :
			?>
			<tr class="form-field term-shopos-tooltip-wrap">
				<th scope="row"><label for="shopos_swatch_tooltip"><?php esc_html_e( 'Hover tooltip override', 'shopos-core' ); ?></label></th>
				<td>
					<input type="text" name="shopos_swatch_tooltip" id="shopos_swatch_tooltip" value="<?php echo esc_attr( $value ); ?>" class="regular-text" />
					<p class="description">
						<?php esc_html_e( 'Text shown on hover. Leave empty to default to the term name.', 'shopos-core' ); ?>
					</p>
				</td>
			</tr>
			<?php
			...
			?>
			<div class="form-field term-shopos-tooltip-wrap">
				<label for="shopos_swatch_tooltip"><?php esc_html_e( 'Hover tooltip override', 'shopos-core' ); ?></label>
				<input type="text" name="shopos_swatch_tooltip" id="shopos_swatch_tooltip" value="" />
				<p class="description">
					<?php esc_html_e( 'Text shown on hover. Leave empty to default to the term name.', 'shopos-core' ); ?>
				</p>
			</div>
	...
	private function render_image_field_inner( int $attachment_id, string $preview_url, bool $has_image ): void {
		...
		<input
			type="hidden"
			name="shopos_swatch_image_id"
			id="shopos_swatch_image_id"
			class="shopos-image-field-id"
			value="<?php echo esc_attr( (string) $attachment_id ); ?>"
		/>
		<img
			class="shopos-image-field-preview"
		...
		/>
		<button type="button" class="button shopos-image-field-pick"
			data-frame-title="<?php esc_attr_e( 'Choose swatch image', 'shopos-core' ); ?>"
			data-frame-button="<?php esc_attr_e( 'Use this image', 'shopos-core' ); ?>">
			<?php esc_html_e( 'Choose image', 'shopos-core' ); ?>
		</button>
		<a href="#" class="shopos-image-field-clear" style="display:<?php echo $has_image ? 'inline' : 'none'; ?>;margin-inline-start:8px;">
			<?php esc_html_e( 'Remove image', 'shopos-core' ); ?>
		</a>
		<p class="description">
			<?php esc_html_e( 'Upload an image to use this term as a picture-swatch. When set, image takes precedence over color.', 'shopos-core' ); ?>
		</p>
		<?php
	}
	...
	public function save_field( int $term_id ): void {
		...
		$key = ShopOS_VS_Plugin::color_meta_key();
		if ( isset( $_POST['shopos_swatch_color'] ) ) {
			$raw = wp_unslash( $_POST['shopos_swatch_color'] );
		...
		if ( \ShopOS\Core\Core\Feature_Flags::is_enabled( 'variation_swatches', 'image_swatches' )
			&& isset( $_POST['shopos_swatch_image_id'] )
		) {
			$image_key     = ShopOS_VS_Plugin::image_meta_key();
			$attachment_id = absint( wp_unslash( $_POST['shopos_swatch_image_id'] ) );
		...
		if ( \ShopOS\Core\Core\Feature_Flags::is_enabled( 'variation_swatches', 'tooltip' )
			&& isset( $_POST['shopos_swatch_tooltip'] )
		) {
			$tooltip_key = ShopOS_VS_Plugin::tooltip_meta_key();
			$raw         = (string) wp_unslash( $_POST['shopos_swatch_tooltip'] );
	...
	public function render_column( string $content, string $column_name, int $term_id ): string {
		...
		if ( \ShopOS\Core\Core\Feature_Flags::is_enabled( 'variation_swatches', 'image_swatches' ) ) {
			$image_url = ShopOS_VS_Plugin::term_image_url( $term_id, 'thumbnail' );
			if ( '' !== $image_url ) {
				return sprintf(
		...
		$hex = ShopOS_VS_Plugin::term_color( $term_id );
		if ( '' === $hex ) {
...
endif; // class_exists ShopOS_VS_Admin

 ...
 * submits via AJAX to our wc-ajax=shopos_shop_add_to_cart endpoint.
 *
 * Expected variables:
 * @var WC_Product $product
 * @var array      $prepared    Output of ShopOS_VS_Archive::prepare_product_data().
...
?>
<div class="shopos-shop-pick<?php
	echo ! empty( $hide_attr_labels ) ? ' shopos-shop-pick--no-labels' : '';
	echo ! empty( $hide_selected )    ? ' shopos-shop-pick--no-selected' : '';
?>"
	...
	<?php if ( $show_price ) :
		...
		?>
		<div class="shopos-shop-pick__price<?php echo $has_range ? ' has-range' : ''; ?>"
			 data-has-range="<?php echo $has_range ? '1' : '0'; ?>"
			 aria-live="polite">
			<span class="shopos-shop-pick__price-prefix"<?php if ( ! $has_range ) : ?> hidden<?php endif; ?>><?php echo esc_html( \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'from_price' ) ); ?></span>
			<span class="shopos-shop-pick__price-value">
				<?php echo wp_kses_post( $prepared['from_price'] ?? '' ); ?>
			</span>
		</div>
	<?php endif; ?>

	<div class="shopos-shop-pick__attrs">
		<?php foreach ( (array) $prepared['attrs'] as $attr ) :
			...
			?>
			<div class="shopos-shop-pick__attr<?php echo $is_color ? ' shopos-shop-pick__attr--color' : ' shopos-shop-pick__attr--button'; ?>"
				 data-attribute-name="<?php echo esc_attr( $name ); ?>"
				 data-max-visible="<?php echo esc_attr( $hide_after ); ?>">
				<div class="shopos-shop-pick__attr-head">
					<span class="shopos-shop-pick__attr-label"><?php echo esc_html( $label ); ?>:</span>
					<span class="shopos-shop-pick__attr-selected" data-default-text="<?php echo esc_attr( \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'choose_option' ) ); ?>">
						<?php
						if ( '' !== $selected ) {
				...
				<div class="shopos-shop-pick__opts">
					<?php foreach ( $options as $i => $opt ) :
						$value       = (string) ( $opt['v'] ?? '' );
						$display     = (string) ( $opt['n'] ?? $value );
						$hex_raw     = (string) ( $opt['hex'] ?? '' );
						$safe_hex    = ShopOS_VS_Plugin::sanitize_hex_color( $hex_raw );
						$is_light    = $safe_hex && ShopOS_VS_Plugin::is_light_hex( $safe_hex );
						...
						$classes     = [ 'shopos-shop-pick__opt' ];
						if ( $is_image ) {
							$classes[] = 'shopos-shop-pick__opt--image';
						} elseif ( $is_color ) {
							$classes[] = 'shopos-shop-pick__opt--color';
						} else {
							$classes[] = 'shopos-shop-pick__opt--button';
						}
					...
					<?php if ( $total > 1 ) :
						...
						?>
						<button type="button"
								class="shopos-shop-pick__more"
								aria-expanded="false"
								data-count="<?php echo esc_attr( $overflow_count ); ?>"
								hidden>
							<span class="shopos-shop-pick__more-label">+<?php echo absint( $overflow_count ); ?></span>
						</button>
					<?php endif; ?>
				</div>
			</div>
	...
	<button type="button"
			class="shopos-shop-pick__add"
			disabled
			aria-disabled="true">
		<?php echo esc_html( \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'add_to_cart' ) ); ?>
	</button>
	<?php
	// NOTE (1.6.4): the per-card ".shopos-shop-pick__message" element was
	...
	// assets/js/shopos-shop-swatches.js, so card geometry is unaffected

 ...
 * `.shopos-shop-pick` wrapper so existing CSS (card geometry, toast
 * stack, Add-to-cart button styling) applies without duplication. The
 * modifier class `.shopos-shop-pick--simple` is what the JS handler
 * keys off to skip the variation matcher.
 *
 * Expected variables:
 * @var WC_Product $product
 * @var array      $prepared    Output of ShopOS_VS_Archive::prepare_simple_product_data().
...
?>
<div class="shopos-shop-pick shopos-shop-pick--simple"
	...
	<?php if ( $show_price && ! empty( $prepared['price_html'] ) ) : ?>
		<div class="shopos-shop-pick__price">
	...
	<?php if ( $is_purchasable ) : ?>
		<div class="shopos-shop-pick__row">
			<div class="shopos-shop-pick__qty-wrap">
				<button type="button"
						class="shopos-shop-pick__qty-btn shopos-shop-pick__qty-btn--plus"
						aria-label="+"
						tabindex="-1">+</button>
				<input type="number"
					   class="shopos-shop-pick__qty"
					   value="<?php echo esc_attr( $min_qty ); ?>"
					   min="<?php echo esc_attr( $min_qty ); ?>"
					   <?php if ( $max_qty > 0 ) : ?>max="<?php echo esc_attr( $max_qty ); ?>"<?php endif; ?>
					   step="1"
					   inputmode="numeric"
					   aria-label="<?php echo esc_attr( \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'quantity' ) ); ?>" />
				<button type="button"
						class="shopos-shop-pick__qty-btn shopos-shop-pick__qty-btn--minus"
						aria-label="−"
						tabindex="-1">−</button>
			</div>
	...
	<?php else : ?>
		<button type="button"
				class="shopos-shop-pick__add is-oos"
				disabled
				aria-disabled="true">
			<?php echo esc_html( \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'out_of_stock' ) ); ?>
		</button>
	<?php endif; ?>
</div>

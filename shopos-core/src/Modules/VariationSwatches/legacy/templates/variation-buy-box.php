...
foreach ( $attributes as $attribute_name => $options ) {
	...
	$options        = ShopOS_VS_Plugin::reorder_options_to_match_terms( (int) $product_id, $taxonomy, (array) $options );
	...
	foreach ( $options as $option ) {
		...
		if ( $taxonomy ) {
			...
			if ( $term && ! is_wp_error( $term ) ) {
				$name = $term->name;
				$hex  = \ShopOS\Core\Modules\VariationSwatches\Color_Sampler::resolve_term_color( (int) $term->term_id, (int) $product_id );
...
// JS swaps `.shopos-pdp-price__value` HTML on `found_variation`, restores
...
<p class="price shopos-pdp-price"
   ...
   aria-live="polite">
	<span class="shopos-pdp-price__prefix"<?php if ( ! $pdp_has_range ) : ?> hidden<?php endif; ?>><?php echo esc_html( \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'from_price' ) ); ?></span>
	<span class="shopos-pdp-price__value"><?php echo wp_kses_post( $pdp_min_html ); ?></span>
</p>

<?php do_action( 'woocommerce_before_add_to_cart_form' ); ?>

<form class="variations_form cart shopos-buy-box"
	...
	<?php else : ?>
		...
		<div class="shopos-variations">
			<?php foreach ( $prepared as $attribute_name => $p ) : ?>
				<div class="shopos-variation<?php echo $p['is_color'] ? ' shopos-variation--color' : ' shopos-variation--button'; ?>"
					 data-attribute_name="<?php echo esc_attr( $p['input_name'] ); ?>">
					<div class="shopos-variation__head">
						<span class="shopos-variation__label"><?php echo esc_html( $p['label'] ); ?>:</span>
						<span class="shopos-variation__selected" data-default-text="<?php echo esc_attr( \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'choose_option' ) ); ?>">
							<?php
							$selected_label = '';
					...
					<div class="shopos-variation__options">
						<?php foreach ( $p['option_items'] as $opt ) :
							...
							<?php if ( $is_image_opt ) : ?>
								<button type="button"
										class="shopos-swatch shopos-swatch--image<?php echo $is_selected ? ' is-selected' : ''; ?>"
									...
									<span class="shopos-swatch__img" aria-hidden="true"
							...
							<?php elseif ( $p['is_color'] ) :
								...
								$safe_hex      = ShopOS_VS_Plugin::sanitize_hex_color( (string) $opt['hex'] );
								$is_white_like = ShopOS_VS_Plugin::is_light_hex( $safe_hex );
								?>
								<button type="button"
										class="shopos-swatch shopos-swatch--color<?php echo $is_selected ? ' is-selected' : ''; ?><?php echo $is_white_like ? ' is-light' : ''; ?>"
										data-value="<?php echo esc_attr( $opt['value'] ); ?>"
										data-name="<?php echo esc_attr( $opt['name'] ); ?>"
										data-hex="<?php echo esc_attr( $safe_hex ); ?>"
										<?php if ( $emit_tooltip ) : ?>data-tooltip="<?php echo esc_attr( $opt_tt ); ?>"<?php endif; ?>
										aria-label="<?php echo esc_attr( $opt['name'] ); ?>">
									<span class="shopos-swatch__dot" aria-hidden="true"<?php if ( $safe_hex ) : ?> style="background-color:<?php echo esc_attr( $safe_hex ); ?>"<?php endif; ?>></span>
									<span class="screen-reader-text"><?php echo esc_html( $opt['name'] ); ?></span>
								</button>
							<?php else : ?>
								<button type="button"
										class="shopos-swatch shopos-swatch--button<?php echo $is_selected ? ' is-selected' : ''; ?>"
										data-value="<?php echo esc_attr( $opt['value'] ); ?>"
										data-name="<?php echo esc_attr( $opt['name'] ); ?>">
		...
		<div class="variations shopos-hidden-variations" aria-hidden="true">
			<?php foreach ( $prepared as $attribute_name => $p ) : ?>
				<div class="shopos-hidden-row" data-attribute_name="<?php echo esc_attr( $p['input_name'] ); ?>">
					<label for="<?php echo esc_attr( $p['input_name'] ); ?>"><?php echo esc_html( $p['label'] ); ?></label>
					<select id="<?php echo esc_attr( $p['input_name'] ); ?>"
							class="shopos-hidden-select"
		...
		<div class="single_variation_wrap">
			<div class="single_variation shopos-single-variation"></div>
			...
			<div class="shopos-actions">
				...
				<div class="shopos-actions__row">
					<button type="submit"
							class="shopos-add-to-cart single_add_to_cart_button button alt disabled wc-variation-selection-needed"
							name="add-to-cart"
		...
		<div class="shopos-sticky-bar" aria-hidden="true">
			<div class="shopos-sticky-bar__inner">
				<div class="shopos-sticky-bar__price" aria-live="polite">
					<span class="shopos-sticky-bar__price-value"></span>
				</div>
				<button type="submit"
						class="shopos-sticky-bar__buy shopos-sticky-bar__buy--atc single_add_to_cart_button disabled wc-variation-selection-needed"
						name="add-to-cart"
						value="<?php echo absint( $product_id ); ?>"
						aria-disabled="true">
					<?php echo esc_html( \ShopOS\Core\Modules\VariationSwatches\Labels::get( 'add_to_cart' ) ); ?>

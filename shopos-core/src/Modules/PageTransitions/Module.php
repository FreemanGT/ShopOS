 ...
 *      Exposed as `window.ShopOSPageTransitions.show()` so other modules
 ...
 * a new module is absent from `shopos_core_modules` → off by default.
 *
...
namespace ShopOS\Core\Modules\PageTransitions;
...
final class Module extends Module_Base {
	...
	const HANDLE = 'shopos-core-page-transitions';
	...
	public function label() {
		return __( 'Page Transitions', 'shopos-core' );
	}
	...
	public function description() {
		return __( 'Smooth page reloads: a loading overlay on filter / search / pagination interactions, plus a cross-fade between pages in supporting browsers.', 'shopos-core' );
	}
	...
	public function settings_schema() {
		return array(
			'loading_label' => array(
				'label'       => __( 'Loading overlay text', 'shopos-core' ),
				'type'        => 'text',
				'default'     => '',
				'description' => __( 'Shown under the spinner while the next page loads. Leave blank for the English default (Loading…).', 'shopos-core' ),
			),
		);
	}
	...
	public function enqueue() {
		...
		wp_localize_script( self::HANDLE, 'ShopOSPTConfig', $this->localized_payload() );
	}

	/**
	 * Build the ShopOSPTConfig JS payload. Extracted so PHPUnit can assert

...
final class Privacy {
	 ...
	 */
	private const KEY = 'shopos-core-restock-notify';
	...
	public function register_exporter( $exporters ) {
		$exporters[ self::KEY ] = array(
			'exporter_friendly_name' => __( 'Restock Notify subscriptions', 'shopos-core' ),
	...
	public function register_eraser( $erasers ) {
		$erasers[ self::KEY ] = array(
			'eraser_friendly_name' => __( 'Restock Notify subscriptions', 'shopos-core' ),
	...
	public function exporter( $email_address, $page = 1 ) {
		...
		foreach ( $rows as $row ) {
			$data[] = array(
				'group_id'    => self::KEY,
				'group_label' => __( 'Restock Notify subscriptions', 'shopos-core' ),
				'item_id'     => 'restock-notify-' . (int) $row->id,
				'data'        => array(
					array( 'name' => __( 'Subscription ID', 'shopos-core' ),   'value' => (int) $row->id ),
					array( 'name' => __( 'Product ID', 'shopos-core' ),        'value' => (int) $row->product_id ),
					array( 'name' => __( 'Variation ID', 'shopos-core' ),      'value' => (int) $row->variation_id ),
					array( 'name' => __( 'Customer Name', 'shopos-core' ),     'value' => (string) $row->customer_name ),
					array( 'name' => __( 'Customer Email', 'shopos-core' ),    'value' => (string) $row->customer_email ),
					array( 'name' => __( 'Status', 'shopos-core' ),            'value' => (string) $row->status ),
					array( 'name' => __( 'Subscribed at', 'shopos-core' ),     'value' => (string) $row->created_at ),
					array( 'name' => __( 'Notified at', 'shopos-core' ),       'value' => (string) ( $row->notified_at ?? '' ) ),
					array( 'name' => __( 'Unsubscribe Token', 'shopos-core' ), 'value' => (string) $row->unsubscribe_token ),
				),
			);
		}
		return array(

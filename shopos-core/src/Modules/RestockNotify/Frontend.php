...
final class Frontend {
	...
	public function enqueue_always() {
		 ...
		 * shortcode. The `shopos_restock_should_enqueue` filter (preserved from the
	...
	public function footer_inject() {
		...
		echo '<div id="shopos-restock-footer-fallback" style="display:none !important;">' . $form . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		?>
		<script>
		(function(){
	...
	private function maybe_render( $product, $context = '', $force = false ) {
		 ...
		 *
		 * Distinct from `shopos_restock_should_enqueue` (which gates the asset load
		 ...
		 *                             'stock_filter', 'footer'.
		 */
		if ( ! apply_filters( 'shopos_core/restock_notify/should_inject', true, $product, $context ) ) {
			return '';
		}
	...
	private function render_form( $product_id, $variation_id = 0, $is_variable = false ) {
		...
		$heading     = (string) shopos_restock_get_option( 'form_heading' );
		$description = (string) shopos_restock_get_option( 'form_description' );
		$button_text = (string) shopos_restock_get_option( 'form_button_text' );
		$success_msg = (string) shopos_restock_get_option( 'form_success_message' );
		$enable_gdpr = 'yes' === shopos_restock_get_option( 'enable_gdpr' );
		$gdpr_text   = (string) shopos_restock_get_option( 'gdpr_text' );
		...
		if ( ! self::$inline_css_printed ) {
			...
			// Mirror of assets/css/frontend.css (token-routed, direction follows the
			// wrapper's dir attribute) — keep the two in sync.
			$inline_style = '<style>
				.shopos-restock-form-wrap{margin:24px 0;text-align:start}
				.shopos-restock-form-card{background:var(--shopos-ui-color-paper,#fff);border:1px solid var(--shopos-ui-color-hairline,#e5e5e5);border-radius:var(--shopos-ui-radius-lg,12px);padding:32px 28px;max-width:480px;text-align:start;font-family:var(--shopos-ui-font-body,-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"Helvetica Neue",Arial,sans-serif)}
				.shopos-restock-form-heading{margin:0 0 6px;font-size:18px;font-weight:600;color:var(--shopos-ui-color-ink,#111)}
				.shopos-restock-form-desc{margin:0 0 20px;font-size:14px;color:var(--shopos-ui-color-ink-muted,#666);line-height:1.6}
				...
				.shopos-restock-error-text{margin:0;padding:10px 14px;font-size:13px;color:var(--shopos-ui-color-danger,#c00);background:var(--shopos-ui-color-danger-soft,#fff5f5);border:1px solid var(--shopos-ui-color-danger-soft,#fdd);border-radius:var(--shopos-ui-radius-md,6px)}
				...
				.shopos-restock-loading .shopos-restock-btn-spinner{display:inline-flex;animation:rsnSpin var(--shopos-ui-motion-spin,.8s) linear infinite}
				...
				.shopos-restock-form-wrap{animation:rsnFadeIn var(--shopos-ui-motion-base,.4s) var(--shopos-ui-ease-out,ease-out)}
		...
		?>
		<div class="<?php echo esc_attr( $wrapper_class ); ?>" dir="<?php echo is_rtl() ? 'rtl' : 'ltr'; ?>" data-product-id="<?php echo esc_attr( $product_id ); ?>" data-variation-id="<?php echo esc_attr( $variation_id ); ?>">
				...
				<div class="shopos-restock-form-fields">
					<div class="shopos-restock-field-row">
						<div class="shopos-restock-field">
							<input type="text" class="shopos-restock-input shopos-restock-name" placeholder="<?php echo esc_attr( $placeholder_name ); ?>" autocomplete="name" />
						</div>
						<div class="shopos-restock-field">
							<input type="email" class="shopos-restock-input shopos-restock-email" placeholder="<?php echo esc_attr( $placeholder_email ); ?>" autocomplete="email" required />
						</div>
					</div>
	...
	/* =================================================================
	   ...
	   COPIED VERBATIM from legacy/includes/class-shopos-restock-frontend.php:513-564
	...
	public function handle_unsubscribe() {
		if ( ! isset( $_GET['shopos_restock_unsubscribe'] ) ) {
			return;
		}

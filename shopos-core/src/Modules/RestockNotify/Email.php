 ...
 *     - `shopos_core/restock_notify/email_args` (filter) — mutate the args
 *       dictionary that drives `build_html()`.
 *     - `shopos_core/restock_notify/before_send` (action) — fires immediately
 *       before `wp_mail()`.
 *
 * @package ShopOSCore
 */
...
final class Email {
	...
	private static function build_html( $a, $subscriber, $kind ) {
		 ...
		 * @param string $kind       'confirmation' or 'notification'.
		 */
		$a = (array) apply_filters( 'shopos_core/restock_notify/email_args', $a, $subscriber, $kind );
...
' . $unsub . '
	...
	private static function send( $to, $subject, $html, $subscriber ) {
		 ...
		 * @param object $subscriber Subscriber row.
		 */
		do_action( 'shopos_core/restock_notify/before_send', $to, $subject, $html, $subscriber );

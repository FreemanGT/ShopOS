...
final class CSV_Exporter {
	 ...
	 * literal string.
	 */
	public const ACTION = 'shopos_restock_notify_csv_export';
	...
	public function handle_export() {
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			wp_die( esc_html__( 'You do not have permission to export subscribers.', 'shopos-core' ), 403 );
		}
	...
	public function build_csv( array $rows ): string {
		$headers = array(
			__( 'Subscription ID', 'shopos-core' ),
			__( 'Product ID', 'shopos-core' ),
			__( 'Variation ID', 'shopos-core' ),
			__( 'Customer Name', 'shopos-core' ),
			__( 'Customer Email', 'shopos-core' ),
			__( 'Status', 'shopos-core' ),
			__( 'Subscribed at', 'shopos-core' ),
			__( 'Notified at', 'shopos-core' ),
			__( 'Unsubscribe Token', 'shopos-core' ),
		);

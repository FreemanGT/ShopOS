...
?>
<?php if ( ! empty( $saved ) ) : ?>
	<div class="notice notice-success inline"><p><?php esc_html_e( 'Filter configuration saved.', 'shopos-core' ); ?></p></div>
<?php endif; ?>

<p class="description">
	<?php esc_html_e( 'Choose which attribute filters appear, their order, and any categories they should be hidden on. Leave everything as-is to keep the automatic behaviour (every attribute shown, categories as a tree).', 'shopos-core' ); ?>
</p>
...
<form method="post" action="<?php echo esc_url( $action_url ); ?>">
	...
	<table class="widefat striped" style="max-width:840px;">
		<thead>
			<tr>
				<th><?php esc_html_e( 'Filter', 'shopos-core' ); ?></th>
				<th><?php esc_html_e( 'Show', 'shopos-core' ); ?></th>
				<th><?php esc_html_e( 'Order', 'shopos-core' ); ?></th>
				<th><?php esc_html_e( 'Hide on categories', 'shopos-core' ); ?></th>
			</tr>
		</thead>
		<tbody>
				...
				<tr>
					...
					<td>
						<?php if ( empty( $categories ) ) : ?>
							<span class="description"><?php esc_html_e( 'No categories found.', 'shopos-core' ); ?></span>
						<?php else : ?>
	...
	<?php submit_button( __( 'Save filter configuration', 'shopos-core' ) ); ?>
</form>
<?php if ( ! empty( $categories ) ) : ?>
	<p class="description"><?php esc_html_e( 'Hold Ctrl (Cmd on Mac) to select more than one category to hide a filter on.', 'shopos-core' ); ?></p>
<?php endif; ?>

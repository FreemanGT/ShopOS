 ...
 * ShopOS → Shop Filters page that lets the shop owner override the
 * auto-derived facet defaults: turn a filter on/off, reorder it, and hide it
 * on chosen categories. Writes the `shopos_core_shop_filters_facet_config`
...
final class Admin_Config_Page {
	...
	const NONCE  = 'shopos_core_shop_filters_save_facet_config';
	const ACTION = 'shopos_core_shop_filters_save_facet_config';
	...
	public function boot() {
		add_action( 'shopos_core/module_page/shop_filters', array( $this, 'render' ), 30 );
	...
	public function render() {
		...
		echo '<h2>' . esc_html__( 'Filter configuration', 'shopos-core' ) . '</h2>';

		if ( empty( $taxonomies ) ) {
			echo '<p>' . esc_html__( 'No indexed attributes yet. Build the index (above) first — the filters you can configure are the ones the index holds.', 'shopos-core' ) . '</p>';
			return;
		}
		...
		include SHOPOS_CORE_PATH . 'src/Modules/ShopFilters/templates/admin-facet-config.php';
	}
	...
	public static function taxonomy_label( $taxonomy ) {
		if ( 'product_cat' === $taxonomy ) {
			return __( 'Categories', 'shopos-core' );
		}

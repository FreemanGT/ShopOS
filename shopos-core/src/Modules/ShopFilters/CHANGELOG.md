...
- Indexer status & sweep refinements (from QA feedback):
  - The ShopOS → Shop Filters status line now shows **"Last refresh"** — the actual time the sweep (or a full reindex) last ran — instead of the internal resume watermark, which during catch-up read as a confusing old product-modified date.
...
- Admin control surface (Phase 6.1 follow-up, by request) — manage everything from ShopOS → Shop Filters without WP-CLI:
  - **Background indexing** toggle — a `settings_schema` checkbox keyed `indexer_enabled` that writes the same option the feature flag reads, so it's the same switch as the ShopOS → Feature Flags entry.
...
- Facet engine (Wave 6, Phase 6.2). Pure, fully unit-tested computation core — still no storefront output:
  ...
  - `Facet_Config` — auto-derives facets from the catalogue's attributes (every `pa_*` as a checkbox, `product_cat` as a tree); filters `shopos_core/shop_filters/{facet_config, is_facet_visible}` let code override.
...
- Custom index table `{prefix}shopos_shop_filter_index` (`product_id`, `taxonomy`, `term_id`, `in_stock`) — a narrow term / category membership table with a per-attribute-value in-stock flag. Price / stock / rating are read from WooCommerce's own `wc_product_meta_lookup`, never duplicated. Auto-installed via `Migrations::run()` on the version bump; dropped on uninstall.
...
- Admin **Reindex all products** tool on the ShopOS → Shop Filters page (offset-paged batches, `manage_woocommerce`).
- Gated behind the `shopos_core_shop_filters_indexer_enabled` feature flag (default **off**). The module itself is disabled by default; enabling it without the flag does nothing.

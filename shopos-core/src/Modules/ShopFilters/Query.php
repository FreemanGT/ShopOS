 ...
 * does not always govern. Tertiary: the ShopOS ProductSlider widget's
 * `shopos_core/product_slider/query_args` filter, for grids that build their
...
final class Query {
	...
	public function register() {
		...
		add_filter( 'shopos_core/product_slider/query_args', array( $this, 'constrain_slider_query' ), 20, 2 );
	}
	...
	/**
	 * Constrain a ShopOS ProductSlider widget's own wc_get_products() query to
	...
	public function default_catalog_orderby( $default ) {
		$setting = (string) get_option( 'shopos_core_shop_filters_default_sort', '' );

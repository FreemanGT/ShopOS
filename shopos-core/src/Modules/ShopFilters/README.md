...
- **`Database`** — owns the index table `{prefix}shopos_shop_filter_index`
  (`product_id, taxonomy, term_id, in_stock`). Auto-installed by
  `ShopOS\Core\Core\Migrations::run()` on the version bump; dropped on uninstall.
...
- **`Admin_Page`** — the "Reindex all products" tool on ShopOS → Shop Filters.

## Feature flag

`shopos_core_shop_filters_indexer_enabled` (default off) gates the indexer hooks, scheduling
and the admin tool. Enable:

```
wp option update shopos_core_shop_filters_indexer_enabled 1
```

Disable (rollback):

```
wp option update shopos_core_shop_filters_indexer_enabled 0
```

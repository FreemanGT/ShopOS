...
final class Indexer {
	...
	const QUEUE_OPTION     = 'shopos_core_shop_filters_dirty_queue';
	const WATERMARK_OPTION = 'shopos_core_shop_filters_last_sweep_gmt';
	const LAST_RUN_OPTION  = 'shopos_core_shop_filters_last_run_gmt';
	const REV_OPTION       = 'shopos_core_shop_filters_index_rev';
	const REV_AT_OPTION    = 'shopos_core_shop_filters_index_rev_at';
	const REV_DEBOUNCE     = 60; // min seconds between order-churn rev bumps (A4).
	const DRAIN_HOOK       = 'shopos_core_shop_filters_drain_queue';
	const RECONCILE_HOOK   = 'shopos_core_shop_filters_reconcile';
	const CRON_SCHEDULE    = 'shopos_shop_filters_5min';
	const BATCH_SIZE       = 50;
	...
	public function ensure_scheduled() {
		...
		if ( function_exists( 'as_schedule_recurring_action' ) && function_exists( 'as_next_scheduled_action' ) ) {
			if ( ! as_next_scheduled_action( self::RECONCILE_HOOK ) ) {
				as_schedule_recurring_action( time() + self::SWEEP_INTERVAL, self::SWEEP_INTERVAL, self::RECONCILE_HOOK, array(), 'shopos-shop-filters' );
			}
			return;
		}
	...
	private function chain_reconcile() {
		if ( function_exists( 'as_enqueue_async_action' ) ) {
			as_enqueue_async_action( self::RECONCILE_HOOK, array(), 'shopos-shop-filters' );
			return;
		}

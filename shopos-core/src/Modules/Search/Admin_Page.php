 ...
 * ShopOS → Search page. Mirrors the Shop Filters reindex tool: a get-total
...
final class Admin_Page {
	...
	const NONCE          = 'shopos_core_search_reindex';
	const AJAX_GET_TOTAL = 'shopos_core_search_get_total';
	const AJAX_RUN_BATCH = 'shopos_core_search_reindex_batch';
	const BATCH_SIZE     = 50;
	...
	public function boot() {
		add_action( 'shopos_core/module_page/search', array( $this, 'render' ) );
	...
	public function render() {
		...
		$status    = sprintf(
			...
			__( 'Indexed: %1$d products. Last refresh: %2$s. Auto-reindex: %3$s.', 'shopos-core' ),
		...
		<p>
			<button id="shopos-search-start" class="button button-primary"><?php esc_html_e( 'Reindex all products', 'shopos-core' ); ?></button>
			<button id="shopos-search-stop"  class="button" disabled><?php esc_html_e( 'Stop', 'shopos-core' ); ?></button>
		</p>
		<div id="shopos-search-progress" style="margin-top:20px;display:none;">
			<p id="shopos-search-status"></p>
			<div style="background:#ddd;width:100%;height:20px;border-radius:3px;overflow:hidden;">
				<div id="shopos-search-bar" style="background:#2271b1;height:100%;width:0%;transition:width 0.3s;"></div>
			</div>
			<p id="shopos-search-counts" style="margin-top:10px;font-family:monospace;"></p>
		</div>
		<script>
		(function () {
			...
			var startBtn = document.getElementById('shopos-search-start');
			var stopBtn  = document.getElementById('shopos-search-stop');
			var progress = document.getElementById('shopos-search-progress');
			var statusEl = document.getElementById('shopos-search-status');
			var bar      = document.getElementById('shopos-search-bar');
			var counts   = document.getElementById('shopos-search-counts');

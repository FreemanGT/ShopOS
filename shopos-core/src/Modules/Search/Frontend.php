 ...
 * `[shopos_search]` shortcode field (and most native theme search boxes) match
 * — progressive enhancement, the native form still submits with JS off.
 *
 * Also provides a `[shopos_search]` shortcode that prints a standalone product
...
final class Frontend {
	...
	const HANDLE = 'shopos-core-search';
	...
	public function register() {
		...
		add_shortcode( 'shopos_search', array( $this, 'render_form' ) );
	}

	/**
	 * `[shopos_search]` — a standalone product search box. Renders a native GET
	...
	public function enqueue() {
		...
		wp_localize_script( self::HANDLE, 'ShopOSSearch', $this->localized_payload() );
	}

	/**
	 * Build the ShopOSSearch JS payload. Extracted so PHPUnit can assert the
	...
	public function localized_payload() {
		return array(
			...
			// The `[shopos_search]` box renders input[type=search][name=s]; the

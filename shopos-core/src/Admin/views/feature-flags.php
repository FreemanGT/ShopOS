<?php
/**
 * ShopOS → Feature Flags page view.
 *
 ...
 * `shopos_core/feature_flag/...` code filter render disabled — the DB
...
<div class="wrap shopos-wrap">
	<h1><?php esc_html_e( 'Feature Flags', 'shopos-core' ); ?></h1>

	<?php if ( $fr_just_saved ) : ?>
		<div class="notice notice-success is-dismissible"><p><?php esc_html_e( 'Feature flags saved.', 'shopos-core' ); ?></p></div>
	<?php endif; ?>

	<p class="description" style="max-width:48em;">
		<?php esc_html_e( 'These are roll-out switches for features that ship turned off. Most are meant to be flipped once per site and left alone — flip one only if you know what it gates. Each entry below explains its effect; the matching WP-CLI command is shown for reference.', 'shopos-core' ); ?>
	</p>

	<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="margin-top:16px;">
		<?php wp_nonce_field( 'shopos_save_feature_flags' ); ?>
		<input type="hidden" name="action" value="shopos_save_feature_flags"/>
					...
					<tr>
						...
						<td>
							<label>
								...
								<?php echo esc_html( $fr_enabled ? __( 'Enabled', 'shopos-core' ) : __( 'Disabled', 'shopos-core' ) ); ?>
							</label>
							...
							<p class="description">
								...
								&nbsp;·&nbsp;<?php /* translators: %s: version string */ printf( esc_html__( 'since %s', 'shopos-core' ), esc_html( $fr_flag['since'] ) ); ?>
								<?php if ( ! empty( $fr_flag['shared'] ) ) : ?>
									&nbsp;·&nbsp;<em><?php esc_html_e( 'shared switch — one toggle, several sub-features', 'shopos-core' ); ?></em>
								<?php endif; ?>
							</p>
							<?php if ( $fr_forced ) : ?>
								<p class="description" style="color:#996800;">
									<?php esc_html_e( 'A code-level filter is forcing this flag — the checkbox is disabled because the database option would be ignored.', 'shopos-core' ); ?>
								</p>
							<?php endif; ?>
						</td>
					</tr>
		...
		<?php submit_button( __( 'Save feature flags', 'shopos-core' ) ); ?>
	</form>
	...
	<p class="description" style="max-width:48em;">
		...
		echo wp_kses(
			...
			sprintf( __( 'For staging / dev you can also force a flag from code without touching the database: add a filter on %s in a mu-plugin.', 'shopos-core' ), '<code>shopos_core/feature_flag/{module}/{feature}</code>' ),
			array( 'code' => array() )
		);
		?>
	</p>

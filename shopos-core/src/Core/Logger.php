...
final class Logger {
	...
	public static function log( $message, $level = 'info' ) {
		 ...
		 */
		$filtered = apply_filters( 'shopos_core/logger/entry', $entry, (string) $message, $level );
		if ( is_array( $filtered ) ) {
			$entry = $filtered;
		}

		if ( defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ) {
			error_log( '[ShopOSCore][' . $level . '] ' . $message );
		}
		 ...
		 */
		do_action( 'shopos_core/logger/written', $entry, $log );
	}

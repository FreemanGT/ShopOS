...
abstract class Module_Base implements Module_Interface {
	 ...
	 * shopos_core.<module_id>.<setting>.
	 *
	 * @var string
	 */
	protected $option_prefix = 'shopos_core';
	...
	public function health() {
		if ( ! $this->is_enabled() ) {
			return array( 'level' => 'amber', 'message' => __( 'Disabled', 'shopos-core' ) );
		}
		if ( ! $this->dependencies_met() ) {
			return array( 'level' => 'red', 'message' => __( 'Missing dependency', 'shopos-core' ) );
		}
		return array( 'level' => 'green', 'message' => __( 'Active', 'shopos-core' ) );
	}
	...
	public function asset_url( $path = '' ) {
		return SHOPOS_CORE_URL . 'src/Modules/' . $this->module_folder() . '/assets/' . ltrim( $path, '/' );
	}
	...
	public function asset_min_url( $path ) {
		...
		$fs_base = SHOPOS_CORE_PATH . 'src/Modules/' . $folder . '/assets/';

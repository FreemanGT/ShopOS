 ...
 *
 * Kept minimal — the heavy lifting is inside ShopOS Core modules.
 *
 * @package ShopOSTheme
 */
...
// The `:not(.cs-track):not(.cs-grid)` guards exclude ShopOS slider/grid
...
add_action(
	...
	static function () {
		...
		$cols = get_theme_mod( 'shopos_shop_cols_mobile', 'default' );
		if ( ! in_array( $cols, array( '1', '2', '3', '4' ), true ) ) {
			return;
		}
		printf(
			'<style id="shopos-shop-cols-mobile">@media (max-width:767px){.woocommerce ul.products:not(.cs-track):not(.cs-grid),.woocommerce ul.products.elementor-grid{display:grid !important;grid-template-columns:repeat(%d,minmax(0,1fr)) !important;}}</style>' . "\n",
			(int) $cols
		);
	}
);

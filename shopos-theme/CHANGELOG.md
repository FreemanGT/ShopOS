...
- **ShopOS v2 design migration ("The Quiet Boutique") — theme-side.** The bulk of the DESIGN.md v2 upgrade (§20 checklist). All backward-compatible; module CSS consumes tokens with on-value fallbacks and no flags are flipped.
  - **Token layer** (`assets/css/shopos-tokens.css` rewritten to the v2 frontmatter spec): added `--shopos-ui-text-display` (52→120px), `leading-flat`, `palette-forest #3f6b4f`, the four contrast-safe `*-text` tokens (`danger/success/warning/info-text`), the `.is-inverted` semantic remap block, `container-wide 1560px`, and fluid `container-pad`/`section-gap`; retuned the fluid type scale + `leading-tight`/`base` + `tracking-tight`; **deleted** `radius-sm`/`radius-xl`/`ease-bounce`/`card-hover-scale`/`input-focus-ring`/`tracking-caps` (every consumer migrated first). `--shopos-ui-font-display` aliased to `font-body`.
  - **One type family** (Heebo, variable 300–700): `assets/css/shopos-fonts.css` drops the Assistant + Rubik `@font-face` blocks and the unused woff2/OFL files; the `design-tokens.php` bridge font fallbacks de-Assistant/Rubik'd; `enqueue_assets()` comment updated.
  - **`assets/css/shopos.css`** swept to the v2 doctrine: single `:focus-visible` ink outline (removed the input focus box-shadow + the state border-radius morph), button `:hover` `accent-hover` bg + `:active` `accent-active`, badge `-soft` fills use the `-text` tokens, suite-wide `tabular-nums` on prices. **New primitives/patterns** (additive): `.shopos-ui-iconbtn` (§9.2), `.shopos-ui-chip`/`-note`/`-toast` (§9.5/§9.8), `.shopos-ui-display` + `.shopos-ui-hero` (split + full-bleed) + `.shopos-ui-section-head` + `.shopos-ui-container--wide`, and the `.is-inverted` surface mechanism.
  - **`shopos-rtl.css`**: the Hebrew Label Voice (word-spacing 0.06em + size-step + opened `leading-flat`/`leading-tight` under `.shopos-rtl`); display tracking zeroed under RTL.
  - **Footer + PLP:** the chrome footer carries `.is-inverted` (always inverted, §12.6); the ShopOS Line PLP (`shopos-plp.css`) uses the §12.3 responsive grid (2/3/4 at 640/1024, `minmax(0,1fr)`, WC float neutralized) — verified in wp-env. Account/cart/checkout/chrome skins reconciled to the 640/1024 breakpoint canon.
  - Requires shopos-core 1.54.0+ (the `Design::kit_slots` display-slot default and the tokens:check guard).

## [1.20.0] — 2026-07-20

- **ShopOS Line checkout skin** (§11-B surface 5), behind `theme.style_checkout` (Core flag, default OFF). **Skin-only — the family exception** (Ruling 9 resolved-as-moot, owner 2026-07-20): this forks **no** templates and adds **no** `woocommerce_locate_template` arm. `ShopOS_Theme::enqueue_assets()` enqueues a single new `assets/css/shopos-checkout.css` on `is_checkout()` when the flag is on; WooCommerce keeps ownership of every checkout field, nonce, and payment gateway, and the stylesheet only restyles them by their stable classes (`form.checkout` / `#order_review` / `#place_order` for the shortcode; `.wc-block-checkout` best-effort for the block — restyle only, no relayout). Because it is skin-only it works on **both** transports, so no per-store content-migration is ever required. Skin-light `--shopos-ui-*` tokens with on-palette fallbacks, RTL-safe logical properties, 768px canon breakpoint, reduced-motion; **no JS** (WC owns checkout behaviour). A once-per-request §11 Ruling 10 fonts-off warning fires on a checkout request when `theme.fonts_selfhost` is off. Flag off ⇒ the current checkout render with zero new assets, byte-identical (§11 Ruling 6). Single pinned flag read via `ShopOS_Theme::checkout_enabled()` (FQCN); Core absent ⇒ hard false. Requires shopos-core 1.52.0+ for the flag.

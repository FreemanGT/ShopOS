<?php
/**
 * ShopOS Line — shipping calculator (§11-B surface 2, theme.template_cart).
 *
 * Forked from WooCommerce templates/cart/shipping-calculator.php @version 9.7.0.
 * Reached only via ShopOS_Theme::locate_cart_template() (flag on). Kept
 * near-verbatim: WooCommerce's `country_select` / `state_select` JS + the
 * `.shipping-calculator-button` toggle bind to these exact classes/ids and the
 * `woocommerce-shipping-calculator` nonce; reskinning is CSS-only via the
 * `.shopos-cart` scope. A single `shopos-cart__calc` class is added to the form.
 *
 * @package ShopOSTheme
 */
...
<form class="woocommerce-shipping-calculator shopos-cart__calc" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">

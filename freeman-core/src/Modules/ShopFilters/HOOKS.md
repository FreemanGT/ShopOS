...
`freeman_core/shop_filters/pre_search_product_ids` and
...
### `freeman_core/shop_filters/facet_cache_ttl` (filter, since 1.21.40)
```php
apply_filters(
    'freeman_core/shop_filters/facet_cache_ttl',
    int $seconds // default 5 * MINUTE_IN_SECONDS (300)
);
```
Filters the time-to-live, in seconds, of the facet-response cache transient
(`freeman_core_sf_q_<hash>`) written by `Query_Builder::query()`. The cache is

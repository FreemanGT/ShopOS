<?php
/**
 * Search admin tool — a "Reindex all products" batch runner rendered on the
 * Freeman → Search page. Mirrors the Shop Filters reindex tool: a get-total
...
final class Admin_Page {
	...
	const NONCE          = 'freeman_core_search_reindex';
	const AJAX_GET_TOTAL = 'freeman_core_search_get_total';
	const AJAX_RUN_BATCH = 'freeman_core_search_reindex_batch';
	const BATCH_SIZE     = 50;
	...
	public function boot() {
		add_action( 'freeman_core/module_page/search', array( $this, 'render' ) );
	...
	public function render() {
		...
		$status    = sprintf(
			/* translators: 1: indexed product count, 2: last refresh time, 3: scheduled state */
			__( 'Indexed: %1$d products. Last refresh: %2$s. Auto-reindex: %3$s.', 'freeman-core' ),
		...
		<p>
			<button id="freeman-search-start" class="button button-primary"><?php esc_html_e( 'Reindex all products', 'freeman-core' ); ?></button>
			<button id="freeman-search-stop"  class="button" disabled><?php esc_html_e( 'Stop', 'freeman-core' ); ?></button>
		</p>
		<div id="freeman-search-progress" style="margin-top:20px;display:none;">
			<p id="freeman-search-status"></p>
			<div style="background:#ddd;width:100%;height:20px;border-radius:3px;overflow:hidden;">
				<div id="freeman-search-bar" style="background:#2271b1;height:100%;width:0%;transition:width 0.3s;"></div>
			</div>
			<p id="freeman-search-counts" style="margin-top:10px;font-family:monospace;"></p>
		</div>
		<script>
		(function () {
			...
			var startBtn = document.getElementById('freeman-search-start');
			var stopBtn  = document.getElementById('freeman-search-stop');
			var progress = document.getElementById('freeman-search-progress');
			var statusEl = document.getElementById('freeman-search-status');
			var bar      = document.getElementById('freeman-search-bar');
			var counts   = document.getElementById('freeman-search-counts');

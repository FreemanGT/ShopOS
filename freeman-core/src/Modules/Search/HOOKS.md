...
### `freeman_core/search/max_results` (filter, since 1.21.40)
```php
apply_filters(
...
### `freeman_core/rate_limit_defaults` (filter, since 1.21.40)
```php
apply_filters(
...
```
Declared in `Freeman\Core\Core\Security::rate_limit()` (core, not Search-owned)

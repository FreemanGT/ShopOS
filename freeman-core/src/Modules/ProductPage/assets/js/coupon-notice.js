 ...
 * wc_price() HTML) on data-fm-coupon-prices. On WooCommerce's
...
( function () {
	...
	function init() {
		...
		var boxes = document.querySelectorAll( '.fm-coupon-notice[data-fm-coupon-prices]' );
		if ( ! boxes.length ) {
			return;
		}

		Array.prototype.forEach.call( boxes, function ( box ) {
			var priceEl = box.querySelector( '[data-fm-coupon-price]' );
			if ( ! priceEl ) {
				return;
			}
			var map;
			try {
				map = JSON.parse( box.getAttribute( 'data-fm-coupon-prices' ) || '{}' );
			} catch ( e ) {
				return;
			}

 ...
 * `[freeman_stock_urgency]` shortcode (legacy alias `[stock_urgency]`),
...
final class Stock_Urgency {
	...
	const HANDLE = 'freeman-core-pp-urgency';
	...
	public function register() {
		...
		add_shortcode( 'freeman_stock_urgency', array( $this, 'shortcode' ) );
	...
	public function badge_for_product( $product ) {
		 ...
		 */
		if ( ! apply_filters( 'freeman_core/product_page/show_stock_urgency', true, $product ) ) {
			return '';
		}
		 ...
		 * @param \WC_Product       $product  Current product.
		 */
		$messages = (array) apply_filters( 'freeman_core/product_page/urgency_messages', $messages, $product );

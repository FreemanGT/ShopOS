 ...
 *
 * Action `freeman_core_quick_view_product` (admin-AJAX per decision §6.3 — no
...
final class Ajax {
	...
	const ACTION = 'freeman_core_quick_view_product';
	const NONCE  = 'freeman_core_quick_view_product';
	...
	public function handle_product() {
		...
		if ( ! $this->is_viewable( $product, $product_id ) ) {
			wp_send_json_error( array( 'message' => __( 'Product not found.', 'freeman-core' ) ), 404 );
		}
		 ...
		 */
		$html = (string) apply_filters( 'freeman_core/quick_view/drawer_html', $html, $product_id );

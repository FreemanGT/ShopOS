/**
 * Freeman Quick View — trigger + slide-in drawer controller.
 *
 ...
 * the inline-end edge (left on RTL). Fires `freeman_core_quick_view_loaded`
...
(function () {
	...
	var CFG = window.FreemanQuickView || {};
	...
	function announceLoaded() {
		...
		// doesn't reach.
		if (window.jQuery) {
			window.jQuery(document).trigger('freeman_core_quick_view_loaded');
		}
		document.dispatchEvent(new CustomEvent('freeman_core_quick_view_loaded'));
	}

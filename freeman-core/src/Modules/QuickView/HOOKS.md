...
`freeman_core_quick_view_frontend_enabled` (default `false`). Flag-off boots
nothing.

```bash
wp option update freeman_core_quick_view_frontend_enabled 1   # enable
wp option update freeman_core_quick_view_frontend_enabled 0   # disable (rollback)
```

## Filters

### `freeman_core/quick_view/show_trigger`
Whether the quick-view trigger renders for a product card.

```php
add_filter( 'freeman_core/quick_view/show_trigger', function ( $show, $product ) {
...
### `freeman_core/quick_view/drawer_html`
...
### `freeman_core_quick_view_loaded`
...
`admin-ajax.php?action=freeman_core_quick_view_product` (`wp_ajax_` +
`wp_ajax_nopriv_`). POST `product_id` + `_ajax_nonce` (nonce action
`freeman_core_quick_view_product`). Nonce-checked and per-IP rate-limited
...
`<your-theme>/freeman/quick_view/drawer-content.php`.

## Settings

All storefront strings (trigger aria-label, drawer title, close, full-details
link, loading, error) are editable on Freeman → Quick View
(`freeman_core_quick_view_label_<key>` options); a blank field falls back to
the English default.

## Asset handles

`freeman-core-quick-view` (style + script). Dequeue both to remove the

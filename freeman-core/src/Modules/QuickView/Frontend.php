...
final class Frontend {
	...
	const HANDLE = 'freeman-core-quick-view';
	...
	public function enqueue() {
		...
		wp_localize_script( self::HANDLE, 'FreemanQuickView', $this->localized_payload() );
		...
		$cs_handle  = 'freeman-core-card-slider';
		$cs_fs_base = FREEMAN_CORE_PATH . 'src/Modules/HoverSwap/assets/';
		$cs_url     = FREEMAN_CORE_URL . 'src/Modules/HoverSwap/assets/';
		wp_enqueue_style(
			$cs_handle,
			\Freeman\Core\Core\Module_Base::pick_min_url( $cs_fs_base, $cs_url, 'css/card-slider.css' ),
			array(),
			FREEMAN_CORE_VERSION
		);
		wp_enqueue_script(
			$cs_handle,
			\Freeman\Core\Core\Module_Base::pick_min_url( $cs_fs_base, $cs_url, 'js/card-slider.js' ),
			array(),
			FREEMAN_CORE_VERSION,
			true
		);
	...
	/**
	 * Build the FreemanQuickView JS payload. Extracted so PHPUnit can assert
	...
	public function render_trigger() {
		 ...
		 */
		if ( ! apply_filters( 'freeman_core/quick_view/show_trigger', true, $product ) ) {
			return;
		}

...
**§11-B gate override (owner, 2026-07-20):** the owner elected to **start the §11-B surfaces now**, before the measurable unlock below is met (PDP/PLP have not been flag-on a single day; store #2 is not stood up), and also over the 2026-07-19 design-control pivot's push away from theme-owned templates. The theme-CI-lane precondition (b) **is** genuinely satisfied (Ruling 7.8, `@group theme`). **Surface 1 — header/footer chrome — SHIPPED 2026-07-20** (core 1.47.0 + theme 1.16.0, flag `theme.template_chrome`, default OFF; require-parent passthrough byte-identical + ShopOS chrome verified in wp-env; QA doc `tools/qa/chrome-template.md`). **Surface 2 — cart page — SHIPPED 2026-07-20** (core 1.48.0 + theme 1.17.0, flag `theme.template_cart`, default OFF; the whole cart page theme-owned via a flag-gated `woocommerce_locate_template` redirect — a THIRD mechanism alongside the PDP/PLP `template_include` loader and the chrome child-hierarchy — to `templates/woo/cart/` never `{theme}/woocommerce/`; 7 forked `cart/*` templates with WC hooks/nonces verbatim; flag-off = WC default byte-identical, Ruling 6; only the `[woocommerce_cart]` shortcode cart, so **block-cart stores need a per-store block→shortcode content-migration under Hard Rule #3 — the Ruling 9 note made concrete**; `CartTemplateTest` `@group theme`; QA doc `tools/qa/cart-template.md`). **Surface 3 — My Account — SHIPPED 2026-07-20** (core 1.50.0 + theme 1.18.0, flag `theme.template_account`, default OFF; reuses the cart's locate filter, **generalized** to the shared `locate_woo_template` — one callback, a claim arm per surface, new surfaces add an arm not a registration; two STRUCTURAL templates forked — `myaccount/my-account.php` shell + `navigation.php` rail — while the account content AND the auth/payment forms are **CSS-skinned not forked** so WooCommerce keeps ownership of every nonce + gateway field; flag-off = WC default byte-identical, Ruling 6; the `[woocommerce_my_account]` shortcode account, no WC My-Account block ⇒ Ruling 9 low-risk; `AccountTemplateTest` `@group theme`; QA doc `tools/qa/account-template.md`). **Surface 4 — search-results — SHIPPED 2026-07-20** (core 1.51.0 + theme 1.19.0, flag `theme.template_search`, default OFF; the shared `template_include` loader gains a second claim arm — the mirror-positive of the PLP loader's Ruling 2 search refusal, provably disjoint — rendering `templates/woo/search-results.php`; the always-on `Results_Query` owns the ranked query so the template is render-only; flag-off byte-identical, Ruling 6; `ThemeTemplateLoaderTest` `@group theme`). **Surface 5 — checkout skin — SHIPPED 2026-07-20** (core 1.52.0 + theme 1.20.0, flag `theme.style_checkout`, default OFF; **SKIN-ONLY — the Ruling 9 checkout pin RESOLVED-AS-MOOT (owner, 2026-07-20)**: a flag-gated `is_checkout()` stylesheet enqueue, no forked partials, working on both block and shortcode checkout, so no per-store content-migration is required — see Ruling 9 resolution; `CheckoutSkinTest` `@group theme`; QA doc `tools/qa/checkout-skin.md`). **Surface 6 — transactional emails — SHIPPED 2026-07-20 — CLOSES §11-B** (core 1.53.0, flag `theme.style_emails`, default OFF; **Core-side, never the theme** per the ownership map — `Email_Skin` booted from `Plugin::boot()` at method scope hooks `woocommerce_email_styles` flag-on only and appends email-safe CSS Emogrifier inlines, skin-only like checkout so it forks no email templates and chases no WC email `@version`; cron-safe by construction; email clients drop CSS custom properties/`@media`/logical props so the skin is literal hex/px, not `--shopos-ui-*` tokens; flag-off byte-identical, Ruling 6; `EmailSkinTest` `@group theme`; QA doc `tools/qa/email-skin.md`). **§11-B is now COMPLETE — all six deferred surfaces built, every one default OFF.** The measurable unlock below remains the record for the *flip-on-a-live-store* decision even though the *build* gate was waived; the per-surface flag-flip acceptance (render-diff identity, RTL, owner screenshots) is the remaining §11-B work.
...
**RESOLVED (owner, 2026-07-20) — pin resolved-as-MOOT, not pinned to a transport.** At §11-B the pin was re-examined against three surfaces of evidence: (1) the §11-B mechanism (`woocommerce_locate_template` → forked partials) can *only* skin the classic `[woocommerce_checkout]` shortcode — block checkout is a React-rendered `wp:woocommerce/checkout` block with no partials to fork, so "theme-owned checkout via the established mechanism" ≡ "commit to shortcode checkout"; (2) the **My Account surface (§11-B surface 3) already established the robust doctrine for gateway/nonce markup** — fork only the *structural* shells, **CSS-skin the auth/payment forms** so WooCommerce keeps ownership of every nonce + gateway field; checkout is the most gateway-heavy surface in the suite and wants exactly that; (3) WooCommerce 2026 posture — `[woocommerce_checkout]` is **not deprecated** and still supported, block cart/checkout are default-for-new-installs since 8.3 (Nov 2023) but classic still dominates on plugin compat, **no deprecation timeline announced** (the pin's revisit trigger has not fired). **Ruling: the §11-B checkout surface is SKIN-ONLY — a flag-gated `is_checkout()` stylesheet, no forked checkout partials — which works on BOTH block and shortcode checkout, so `cart_checkout_blocks` stays declared `true` and NO per-store content-migration is ever required.** The transport choice stops mattering; the pin is dissolved, not decided. Ceiling accepted: the theme can *restyle* but not *restructure* checkout layout — the deliberate trade for keeping WC's gateway/nonce markup untouched. If a future store genuinely needs structural checkout ownership, that reopens Option B (fork shortcode + per-store block→shortcode migration under Hard Rule #3) as a scoped, per-store decision — it is not the v1 §11-B checkout.

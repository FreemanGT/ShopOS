...
| `freeman_core_quick_view_frontend_enabled` | `false` | Wave 7.2 (1.13.0). Flag ON: the QuickView module's entire storefront surface — the per-card trigger button (`woocommerce_after_shop_loop_item`, covering the Elementor Pro archive grid and the Freeman ProductSlider identically), the `freeman-core-quick-view` CSS/JS assets, the footer drawer shell (printed only on pages that rendered a trigger), and the public admin-AJAX endpoint `freeman_core_quick_view_product` (nonce + per-IP rate limit; serves only published, catalog-visible products). The drawer slides in from the inline-end edge (left on RTL) and renders the standard single-product summary, so VariationSwatches' buy-box swap lights up unaided; `freeman_core_quick_view_loaded` fires after inject for re-init listeners. Flag OFF: `Module::boot()` returns before constructing anything — no hooks, no markup, no assets, no endpoint. |

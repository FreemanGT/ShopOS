...
| `shopos_core_perf_probe_enabled` | `false` | Phase-3 perf-budget (1.38.0). Flag ON lets a storefront request that carries `?shopos_perf=1` respond with three diagnostic headers — `X-ShopOS-Queries` (get_num_queries), `X-ShopOS-Render-Ms` (wall time since `$timestart`), `X-ShopOS-Mem-MB` (peak memory) — emitted at `shutdown:0` behind an output buffer opened at `template_redirect:0` so headers stay sendable (before WP core's `wp_ob_end_flush_all` at shutdown:1). Read by `tools/perf-budget.php` against the committed `tools/perf-budgets.json` per-template budgets. Numbers only — no query text, no paths, nothing sensitive. Flag OFF **or** param missing: `Perf::boot()` registers zero listeners — byte-identical responses. Rollback: `wp option update shopos_core_perf_probe_enabled 0` (or `wp shopos flags set perf.probe off`). |
| `shopos_core_design_panel_enabled` | `false` | Phase-3 §9 (1.35.0). Flag ON registers the **ShopOS → Design** admin page (accent-preset picker + a curated ~8-token allow-list: accent/ink/ink-soft/paper/paper-alt/hairline colours + one corner-radius) and the front-end emit (`Design::emit_tokens()` on `wp_enqueue_scripts` prio 30 — after the theme token bridge — writing a `:root{ --shopos-ui-*: … }` block via an inline-only `wp_add_inline_style`, depending on the theme's `shopos-tokens` handle when present so it wins). Only tokens the owner actually changed are emitted (default accent + blank colours ⇒ empty ⇒ nothing enqueued), so even flag-ON-but-untouched is byte-identical. Values pass the same shape-sanitiser the theme bridge uses; `build_css` re-validates. Additive emit kill-switch filter `shopos_core/design/tokens_css_enabled` (default true). Flag OFF: no admin page, no `wp_enqueue_scripts` listener, no CSS — byte-identical to pre-1.35.0. Rollback: `wp option update shopos_core_design_panel_enabled 0`. |
| `shopos_core_theme_fonts_selfhost_enabled` | `false` | ShopOS Line §11.4 row 3 (core 1.42.2 + theme 1.11.30/1.11.31). Flag ON: the ShopOS theme enqueues `assets/css/shopos-fonts.css` — self-hosted variable-weight woff2 for Heebo/Assistant/Rubik (hebrew+latin subsets shipped in 1.11.30, inert until this flag) — and suppresses the Elementor kit's Google Fonts print (`elementor/frontend/print_google_fonts` → false), so storefront typography no longer depends on the kit loading. The theme reads the flag through the §11 Ruling 4 pinned path (`class_exists( \ShopOS\Core\Core\Feature_Flags::class ) ? is_enabled( 'theme', 'fonts_selfhost' ) : false` — FQCN, the theme is unnamespaced); Core absent ⇒ hard false. Flag OFF: nothing enqueues, kit fonts print as today — byte-identical. **Permanent kill-switch (decisions §11 Ruling 4) — exempt from graduation sweeps**; this flag must be ON before any theme template flag flips (Ruling 10). Companion (unflagged, inert-by-default): `Design::kit_slots()` de-hardcodes the `sk_type_12`/`sk_type_2` typography-slot mapping behind the `shopos_core_theme_kit_slots` Core option + `shopos_core/theme/kit_slots` filter (no UI; defaults emit nothing). Rollback: `wp option update shopos_core_theme_fonts_selfhost_enabled 0` (or `wp shopos flags set theme.fonts_selfhost off`). |
| `shopos_core_theme_template_pdp_enabled` | `false` | ShopOS Line §11.4 row 4 (core 1.43.0 + theme 1.13.0). Flag ON: Core's ProductPage `Template_Loader::template_file()` resolves the ShopOS theme's own PDP copy at `shopos-theme/templates/woo/single-product.php` (a verbatim copy of the module template — same designed page, owned by the theme) instead of the module copy. Resolution is Core-side via `get_stylesheet_directory()` — no parent-theme fallback, no auto-location by file presence (§11.3; deliberately never `{theme}/woocommerce/`), and the `shopos/product_page/single-product.php` public override still wins over both rungs (Hard Rule #2). Flag ON on a theme without the copy (e.g. Blueprint import onto an older theme) logs one `info` row per request and renders the module copy — recorded intended behavior (Ruling 8). Flag ON while `theme.fonts_selfhost` is OFF logs one `warning` row per request (Ruling 10 — fonts flip precondition). Flag OFF: the Core module template renders as today, byte-identical; the ProductPage module toggle stays the outer kill-switch. **Permanent kill-switch (decisions §11 Ruling 4) — exempt from graduation sweeps; Core's Template_Loader and its module copy are permanent (they ARE the rollback, Ruling 5).** Rollback: `wp option update shopos_core_theme_template_pdp_enabled 0` (or `wp shopos flags set theme.template_pdp off`). |
| `shopos_core_theme_template_plp_enabled` | `false` | ShopOS Line §11.4 row 5 (core 1.44.0 + theme 1.14.0). Flag ON: the shared theme loader (`shopos-theme/inc/class-shopos-template-loader.php`, THE single theme-side `template_include` filter at 9999, §11.3) renders the shop page and product-taxonomy archives from the theme's classic template at `shopos-theme/templates/woo/archive-product.php` — the first Elementor→PHP conversion. Claim = product-archive main query (`is_shop()` or `is_product_taxonomy()`, attribute archives included — owner ask 5) with BOTH search guards (Ruling 2): never `is_search()`, and never any request search term (`$_GET['s']` — the live Elementor search page is a product archive where `is_search()` is false, the `Results_Query::should_handle` mirror). Resolution via `get_stylesheet_directory()` only — no parent fallback, no auto-location, no public-override rung (PDP's was a pre-existing contract; PLP mints none). Flag ON with the template missing logs one `info` row per request and renders the current template. Flag ON while `theme.fonts_selfhost` is OFF logs one `warning` row per request (Ruling 10). Flag ON on a theme ≤ 1.13.x (e.g. Blueprint import onto an older theme) is a **silent no-op** — the loader itself ships with the template, so nothing exists to log; the template goes live when the theme updates (owner ask 10b). **Flip precondition beyond fonts: ShopOS Digital ≥ 1.7.7 where Digital is active** — older Digital's Query Optimizer forces `no_found_rows` on archive main queries (`qo_no_found_rows_front`, default on), zeroing `wc_get_loop_prop('total')` so the classic grid renders empty with no Logger row pointing at the cause (found live in the row-5 QA window; fixed in Digital 1.7.7). Flag OFF: the current archive render (Elementor Pro Theme Builder on live stores), byte-identical, zero new assets (`shopos-plp.css` enqueues only on claim). **Permanent kill-switch (decisions §11 Ruling 4) — exempt from graduation sweeps.** Rollback: `wp option update shopos_core_theme_template_plp_enabled 0` (or `wp shopos flags set theme.template_plp off`). |

...
- **8.2 — Live AJAX dropdown** — flag `freeman_core_search_dropdown_enabled`. Public rate-limited endpoint + debounced combobox dropdown (image/title/price/link), progressive enhancement. Adds the read query (`Search_Repository::search()` + `Query_Engine` score/where builders, with the LIKE + exact-SKU fallback for short/Hebrew tokens FULLTEXT drops).

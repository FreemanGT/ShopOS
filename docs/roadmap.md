...
- **8.3 — Results page** — flag `freeman_core_search_results_enabled`. `pre_get_posts` drives the native theme search-results grid from the engine's ordered ids; one additive `freeman_core/shop_filters/search_product_ids` filter so Shop Filters facet counts match the grid.

...
| **DRY the Labels resolver + label-field loop** | settings | M | Add `Labels_Base` + `Module_Base::label_fields()`. Land caller-free FIRST (additive), then adopt per module in follow-up PRs to respect the >3-module STOP gate. Option keys + rendered fields stay byte-identical. |
...
| Item | Dim | Effort | Notes |
|---|---|---|---|
| **ShopOS Search widget** | widgets | M | `Search/Widget.php` delegating to the pure `Frontend::render_form()`; lock `get_name()='shopos_search'` at birth. Shortcode stays. |

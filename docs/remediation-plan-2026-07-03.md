...
- Version bump = `freeman-core.php` + `Plugin.php` (core) / `style.css` + `functions.php` (theme, after PR-1) / header + `FD_VERSION` (digital).
...
- `freeman-theme/functions.php`: `FREEMAN_THEME_VERSION` 1.11.25 → 1.11.26 (busts the stale 1.11.26 assets immediately).
- `tools/release.sh bump_theme()`: edit **both** `style.css` and `functions.php` (mirror `bump_core`'s two-file pattern).
- `tools/release.sh bump_core()`: also update `freeman-core/readme.txt` `Stable tag`; set it to the current version now (1.8.3 → current).
...
### PR-2 · CSV formula-injection fix (modern exporter) — `fix(freeman-core)` — size S
...
### PR-3 · CheapestDefaultVariation `options`→`choices` — `fix(freeman-core)` — size XS
...
### PR-4 · freeman-digital `function_exists` static-string bug — `fix(freeman-digital)` — size XS
...
- Tests: freeman-digital has its own harness (`tests/`) — add coverage if the seam is reachable; otherwise verified by review (one-line).

### PR-5 · ProductFeed silent-failure fix — `fix(freeman-core)` — size S
...
### PR-6 · ProductSlider meta-orderby via `wc_product_meta_lookup` — `perf(freeman-core)` — size M
...
### PR-7 · Short-circuit the dead native search query — `perf(freeman-core)` — size S
- `ShopFilters/Query_Builder.php search_product_ids()`: add a `null`-default **pre-filter** (`freeman_core/shop_filters/pre_search_product_ids`) consulted before the `WP_Query`; `Search/Results_Query.php` supplies engine ids there (keeps the existing post-filter for back-compat — Hard Rule #2).
...
### PR-8 · Facet-cache hardening (audit A2 + A3 + A4) — `perf(freeman-core)` — size M
...
### PR-9 · Facet-build efficiency (audit A5 + A6 + A7) — `perf(freeman-core)` — size M
...
### PR-10 · Background-churn reduction (audit B1 + B3 + B4) — `perf(freeman-core)` — size S/M
...
### PR-11 · ShopFilters indexer lighter variation reads (audit B2) — `perf(freeman-core)` — size M — **higher risk**
...
### PR-12 · Grid per-page source of truth — `fix(freeman-core)` — size S
...
### PR-13 · VariationSwatches JS strings through Labels — `fix(freeman-core)` — size S
- `etucart-swatches.js:219`: read `out_of_stock`/`unavailable` from the localized payload (keys already exist in `Labels.php`); extend the module's `wp_localize_script` payload accordingly.
...
### PR-14 · RestockNotify locale correctness — `fix(freeman-core)` — size S
...
### PR-15 · Font-token routing (kill `sk_type_*` in plugin CSS) — `fix(freeman-core)` + `fix(freeman-theme)` — size S
- `freeman-theme/assets/css/freeman-tokens.css`: `--fm-font-body`/`--fm-font-heading` wrap the kit variables (single mapping point — already partially true).
- `VariationSwatches/assets/css/etucart-*.css` (5 sites): `var(--e-global-typography-sk_type_12-…)` → `var(--fm-font-body, inherit)`.
- Theme touched → bump `FREEMAN_THEME_VERSION` (PR-1's fixed tooling handles it). Live QA: typography unchanged on arba4.

---

## Phase 4 — Consistency & hardening

### PR-16 · Z-index token adoption — `fix(freeman-core)` — size S — CSS-only
...
### PR-17 · Guard & comment sweep — `chore(freeman-core)` — size XS — no behavior
...
- `infinite-scroll.js:13`: drop the `bookomers_debug` alias (debug-only surface, not a shipped hook — Hard Rule #2 not implicated).
...
### PR-18 · freeman-digital hardening — `fix(freeman-digital)` — size S
...
### PR-19 · Additive tunability filters — `feat(freeman-core)` — size S
All additive (Hard Rule #1 exception applies — no flags):
- `freeman_core/shop_filters/facet_cache_ttl` (default 5 min).
- `freeman_core/search/max_results` (default 500).
- `Security::rate_limit` defaults filterable once (`freeman_core/rate_limit_defaults`) — replaces the thrice-repeated `30, 60`.
...
- `.gitignore`: commit the pending `.gstack/` line, add `.impeccable/`, drop the stale `!freeman-theme.zip.example`.
- Root `CHANGELOG.md` wording ("both" → "three packages"); create a minimal `freeman-digital/CHANGELOG.md` seeded from git history going forward.
...
### PR-21 · Theme translation sources — `chore(freeman-theme)` — size S
- Recover `freeman-theme-he_IL.po` (from backups, or `msgunfmt` the committed `.mo`) and commit it; generate `freeman-theme.pot` via WP-CLI; wire the theme into the existing `.pot`-regeneration habit.
...
**B-2 · `bookomers-*` rename strategy.** Options: (a) leave permanently as documented back-compat (zero risk, permanent naming debt), (b) dual-class emit (`bookomers-* freeman-is-*`) for 2 minor versions then drop the old, (c) hard rename (breaks any store CSS targeting old classes). **Recommendation: (b).** Affects InfiniteScroll CSS+JS + ProductSlider CSS (3 files, 2 modules — fits one PR once decided).

# ShopOS Codebase Review — April 2026

Full review of `shopos-theme/` + `shopos-core/` + tooling, covering security, performance, structure/architecture, and UI/UX / missing features / quality.
...
- PSR-4 autoloader in `shopos-core.php` with **zero runtime dependencies**.
...
Nonce protects against CSRF, but there's no filter allowing a membership/B2B plugin to reject the request before it runs. WooCommerce core `woocommerce_add_to_cart_validation` does get called, so in practice restricted products are blocked — but the ShopOS handler doesn't document this, and code that assumes "logged-out users can add anything" could slip in.

**Fix:** Add a `do_action( 'shopos/swatches/before_shop_add_to_cart', $product_id, $variation_id )` explicitly so membership plugins can short-circuit, and document the reliance on `woocommerce_add_to_cart_validation`.
...
[`shopos-core/src/Admin/views/tools.php:27-28`](shopos-core/src/Admin/views/tools.php#L27-L28)

```php
<?php if ( isset( $_GET['imported'] ) ) : // phpcs:ignore WordPress.Security.NonceVerification.Recommended
    echo '<div class="notice notice-success"><p>' . esc_html__( 'Legacy data imported.', 'shopos-core' ) . '</p></div>';
endif; ?>
```

Anyone with admin read access who bookmarks `/wp-admin/admin.php?page=shopos-tools&imported=1` sees a "data imported" confirmation that isn't tied to an actual import. Not an exploit — just misleading.
...
[`shopos-core/src/Modules/ProductFeed/Module.php:277-346`](shopos-core/src/Modules/ProductFeed/Module.php#L277-L346)
...
[`shopos-core/src/Modules/VariableStockFix/Module.php:376-401`](shopos-core/src/Modules/VariableStockFix/Module.php#L376-L401)
...
[`shopos-core/src/Modules/VariationSwatches/legacy/includes/class-archive.php:202-227`](shopos-core/src/Modules/VariationSwatches/legacy/includes/class-archive.php#L202-L227)
...
|---|---|---|---|
| shopos-swatches.js | 36 KB | 12 KB | 24 KB |
| shopos-shop-swatches.js | 27 KB | 9 KB | 18 KB |
| infinite-scroll.js | 21 KB | 7 KB | 14 KB |
| shopos-swatches.css | 30 KB | 8 KB | 22 KB |
| shopos-shop-swatches.css | 16 KB | 5 KB | 11 KB |
| **Total** |  | | **~89 KB** |
...
[`shopos-core/src/Modules/VariableStockFix/Module.php:107-113`](shopos-core/src/Modules/VariableStockFix/Module.php#L107-L113)

A bulk stock import that touches 100 variations triggers 100 immediate checks. Compare to ProductFeed's `schedule_debounced( 30 )` pattern ([ProductFeed/Module.php:150-165](shopos-core/src/Modules/ProductFeed/Module.php#L150-L165)).
...
[`shopos-core/src/Modules/InfiniteScroll/assets/js/infinite-scroll.js:512-522`](shopos-core/src/Modules/InfiniteScroll/assets/js/infinite-scroll.js#L512-L522)
...
[`shopos-core/src/Modules/ProductFeed/Module.php`](shopos-core/src/Modules/ProductFeed/Module.php)
...
**ST-03. Settings surface split between ShopOS menu and legacy menus**
[`shopos-core/src/Modules/RestockNotify/Module.php:70`](shopos-core/src/Modules/RestockNotify/Module.php#L70), [`shopos-core/src/Modules/VariationSwatches/Module.php:71`](shopos-core/src/Modules/VariationSwatches/Module.php#L71)

Four modules (Cheapest, Scroll, StockFix, Feed) expose settings in `Settings_Hub`. Restock keeps its legacy top-level "Restock Notify" menu. Swatches keeps its settings under "WC → Settings → Products". Admins have to hunt for two of six modules. The ShopOS dashboard card doesn't link out to either.
...
[`shopos-core/src/Modules/*/Importer.php`](shopos-core/src/Modules/)
...
**Fix:** Extract `ShopOS\Core\Core\Base_Importer` abstract implementing the shared bits; each concrete importer only overrides `const LEGACY_PLUGIN_FILE` and `import()`.

**ST-05. `Plugin::boot()` swallows module boot exceptions silently**
[`shopos-core/src/Core/Plugin.php:132-140`](shopos-core/src/Core/Plugin.php#L132-L140)
...
**Fix:** Store a `shopos_core_boot_failures` transient on catch; read it in `Dashboard::maybe_onboarding_notice`-adjacent code and render a dismissable notice listing which modules failed and their error message (with `esc_html`). Also surface on the dashboard card header (red dot + tooltip).

### 🔵 LOW — 2 findings

**ST-06. Fragmented i18n domains**
`class-shopos-vs-*.php` still calls `load_plugin_textdomain( 'shopos-vs', … )`. ShopOS Core uses `'shopos-core'`. Any string a translator fixes in one domain isn't reflected in the other.
...
**Fix:** Wrap each in `__( '…', 'shopos-core' )`, then `cd shopos-core && wp i18n make-pot . languages/shopos-core.pot` (or the existing tooling path), re-translate `.po`, `msgfmt` to `.mo`.

**UX-02. Swatch templates lack ARIA and keyboard affordances**
[`shopos-core/src/Modules/VariationSwatches/legacy/templates/shop-variation-pick.php`](shopos-core/src/Modules/VariationSwatches/legacy/templates/shop-variation-pick.php), [`variation-buy-box.php`](shopos-core/src/Modules/VariationSwatches/legacy/templates/variation-buy-box.php)
...
[`shopos-core/src/Modules/InfiniteScroll/Module.php`](shopos-core/src/Modules/InfiniteScroll/Module.php), [`assets/js/infinite-scroll.js`](shopos-core/src/Modules/InfiniteScroll/assets/js/infinite-scroll.js)
...
[`shopos-core/src/Modules/VariationSwatches/Module.php`](shopos-core/src/Modules/VariationSwatches/Module.php)

The other five modules clear their crons / transients on deactivate. Swatches doesn't, so disabling the module leaves legacy `shopos_vs_*` transients and any registered hooks active until page-load sees the module is off.

**Fix:** Implement `on_deactivate()` that clears `shopos_vs_*` transients via `$wpdb->options LIKE '_transient_shopos_vs_%'` cleanup.
...
[`shopos-core/src/Core/Legacy_Importer.php:74-78`](shopos-core/src/Core/Legacy_Importer.php#L74-L78)

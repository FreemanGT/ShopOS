...
## core [1.54.0] + theme [1.21.0] — 2026-07-21

- **ShopOS v2 design migration — "The Quiet Boutique."** DESIGN.md rewritten to v2 and the suite migrated to it via the §20 checklist (branch `feat/design-v2-migration`): the `--shopos-ui-*` token layer rewritten (one type family, `text-display`, `palette-forest`, contrast-safe `*-text` tokens, `.is-inverted` remap, fluid rhythm; `radius-sm`/`radius-xl`/`ease-bounce`/`card-hover-scale`/`input-focus-ring`/`tracking-caps` retired), a single `:focus-visible` ink outline, the 640/1024 breakpoint canon, ink-muted compare-at prices + `tabular-nums`, `-text` on every status `-soft` fill, the Hebrew Label Voice, inverted surfaces + inverted footer + editorial band, the statement display/hero/section-header patterns, the `.shopos-ui-iconbtn`/`-chip`/`-note`/`-toast` primitives, and a `tokens:check` drift guard (`tools/tokens-check.php` + `TokensParityTest`). All backward-compatible (token fallbacks, no flags flipped). QuickView/Search iconbtn adoption + the ShopOS Line §12.3 PLP responsive grid browser-verified in wp-env; per-module icon-trigger adoption + the WC-owned PLP/PDP restyle remain owner-live-store follow-ons (DESIGN.md §20). **Not yet published** — readme.txt Stable tag held at 1.52.0. See the `shopos-core` / `shopos-theme` CHANGELOGs for the per-package detail.
